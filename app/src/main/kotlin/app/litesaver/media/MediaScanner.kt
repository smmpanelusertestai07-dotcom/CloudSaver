    ...
    fun queryAll(): List<Found> {
        ...
        for (volume in volumes) {
            try {
                ...
                LiteLog.log(context, "scan", "images $volume failed: ${e.message}")
            }
            try {
                queryCollection(MediaStore.Video.Media.getContentUri(volume), isVideo = true, out)
            } catch (e: Exception) {
                LiteLog.log(context, "scan", "videos $volume failed: ${e.message}")
            }
        }
        return out
    }
    ...
    private fun queryCollection(
        ...
        context.contentResolver.query(collection, projection, null, null, null)?.use { c ->
            ...
            while (c.moveToNext()) {
                ...
                if (rel != null && rel.startsWith("Pictures/LiteSaver")) continue
