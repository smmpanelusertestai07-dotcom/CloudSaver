...
import app.litesaver.core.logic.ReleasePlanner
...
class Releaser(private val context: Context, private val db: AppDb) {
    ...
    suspend fun releaseBatch(
        ...
        val volumeName = app.litesaver.util.Volumes
    ...
    suspend fun releaseOne(
        ...
        val itemUri = try {
            ...
            LiteLog.log(context, "release", "insert failed ${row.displayName}: ${e.message}")
            return false
        }
        return try {
            ...
            stageFile.delete()
            LiteLog.log(context, "release", "released $actualName -> $relPath")
            true
        } catch (e: Exception) {
            runCatching { resolver.delete(itemUri, null, null) }
            LiteLog.log(context, "release", "copy failed ${row.displayName}: ${e.message}")
            false
        }
    }
