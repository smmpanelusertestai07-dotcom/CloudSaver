...
/** Guide, part 4: builds on GitHub Actions, phone conditions, limits and permissions. */
internal object GuidePhone {
    ...
    val githubActions = section(
        ...
        info("Prices and limits below are GitHub's, $AS_OF. GitHub can change them; its pages are linked below."),
        table(
        ...
        p("Prices are in US dollars; your bank converts them to rupees ($AS_OF)."),
        p(
            "Minutes on Windows and macOS cost more than Linux ones. GitHub bases its minute multipliers on " +
                "its per-minute rates, where a macOS minute costs about ten times a Linux one ($AS_OF).",
        ),
        bullets(
            "Pay-as-you-go, only with a payment method: Linux $0.006 a minute, Linux arm64 $0.005, Windows " +
                "$0.010, macOS $0.062 ($AS_OF).",
            "Without a payment method, Actions stop at the limit: no surprise bill. Minutes reset monthly.",
        ),
        p("The Usage screen shows your real plan, this month's minutes by system, and builds left."),
        p(
            "Free with private repos covers roughly 200 to 400 Android release builds a month, or 13 to 20 " +
                "iOS builds ($AS_OF; build times vary). Public repos are unlimited, but the code is visible.",
        ),
        tip(
            "Recommendation: Free with private repos for your own apps; make open-source projects public. For " +
                "many iOS builds, add paid minutes on GitHub itself.",
        ),
        p(
            "Why only Actions: it already holds your code, so no new account or company sees it. PocketIDE's " +
                "ready templates (Android, iOS, macOS, Windows, Docker, emulator) run only when started, and " +
                "results come back into Media.",
        ),
        p(
    ...
    val conditions = section(
        ...
        table(
            ...
            row("Data Saver", "Big downloads wait for Wi-Fi."),
        ...
        p(
            "Some phone makers stop background apps on their own. On realme, once: Settings → Battery → App " +
                ...
                "off Optimize battery use, and keep it out of App quick freeze. The app guides you on other brands.",
        ),
        info(
    ...
    val limits = section(
        ...
        table(
            ...
            row("Windows builds", "The Windows runner on Actions"),
            row("x86-64 programs", "An Actions runner"),
            row(
    ...
    val all = listOf(githubActions, conditions, limits, permissions)
}
