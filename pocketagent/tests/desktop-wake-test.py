...
SOURCE = (PROJECT / 'app/src/com/pocketagent/mobile/LinuxService.java').read_text()
...
class DesktopWakeTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        result = subprocess.run(compiler + ['-d', str(cls.path), str(source),
            str(PROJECT / 'app/src/com/pocketagent/mobile/TaskGeneration.java')],
    ...
    def run_case(self, name):
        result = subprocess.run(['java', '-cp', str(self.path),
            'com.pocketagent.mobile.DesktopWakeHarness', name], capture_output=True, text=True, timeout=5)
