...
OUT="$(mktemp -d "${TMPDIR:-/tmp}/pocketagent-media-fixtures.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
...
"${JAVAC[@]}" -encoding UTF-8 -source 8 -target 8 \
  ...
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/ProjectArchive.java" \
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/WorkspaceMediaPaths.java" \
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/WorkspaceMediaStore.java" \
  "$PROJECT_DIR/tests/WorkspaceMediaTest.java"
java -cp "$OUT:$PROJECT_DIR/build/classes:$ANDROID_JAR" com.pocketagent.mobile.WorkspaceMediaTest
