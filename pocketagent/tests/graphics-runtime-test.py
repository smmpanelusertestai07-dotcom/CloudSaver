...
spec = importlib.util.spec_from_file_location("graphics", ASSETS / "pocketagent-graphics.py")
...
''',
        ...
        for name, script in scripts.items():
            ...
            path.chmod(0o755)
        cache = self.root / "var/cache/pocketagent/windows"
        ...
        prefix = self.root / "home/coder/.pocketagent/windows/old/.prefix/user.reg"
        prefix.parent.mkdir(parents=True)
        prefix.write_bytes(b"previous app sign-in")
        return dict(os.environ, PATH=str(fakebin) + ":" + os.environ["PATH"],
                    POCKETAGENT_TEST_ROOT=str(self.root), PD_STATE=str(self.root / "state"),
