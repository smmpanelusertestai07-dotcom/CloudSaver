...
JSON_JAR="${POCKETAGENT_JSON_JAR:?Set POCKETAGENT_JSON_JAR to JSON-Java 20250517}"
if [[ "$(sha256sum "$JSON_JAR" | cut -d' ' -f1)" != '3ea61b2a06e31edf1c91134fe9106b0ebb16628be169f3db75bc7a2b06b45796' ]]; then
  echo 'JSON-Java fixture checksum mismatch.' >&2
  exit 2
fi
OUT="$(mktemp -d "${TMPDIR:-/tmp}/pocketagent-workspace-fixtures.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
...
"${JAVAC[@]}" -encoding UTF-8 -source 8 -target 8 -cp "$JSON_JAR" -d "$OUT" \
  ...
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/WorkspaceViewState.java" \
  "$PROJECT_DIR/tests/WorkspaceViewStateTest.java"
java -cp "$OUT:$JSON_JAR" com.pocketagent.mobile.WorkspaceViewStateTest
