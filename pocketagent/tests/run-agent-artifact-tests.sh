...
OUT="$(mktemp -d "${TMPDIR:-/tmp}/pocketagent-agent-artifacts.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
...
"${JAVAC[@]}" -encoding UTF-8 -source 8 -target 8 -cp "$PROJECT_DIR/build/classes:$ANDROID_JAR" -d "$OUT" \
 ...
 "$PROJECT_DIR/app/src/com/pocketagent/mobile/AgentArtifactPaths.java" \
 "$PROJECT_DIR/tests/AgentArtifactPathsTest.java"
java -cp "$OUT:$PROJECT_DIR/build/classes:$ANDROID_JAR" com.pocketagent.mobile.AgentArtifactPathsTest
