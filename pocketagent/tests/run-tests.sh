...
# app/src relative to the working directory, so `bash pocketagent/tests/run-tests.sh` from the
...
"${JAVAC[@]}" -encoding UTF-8 -source 8 -target 8 -d "$OUT" \
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/TaskGeneration.java" \
  "$PROJECT_DIR/tests/TaskGenerationTest.java"
java -cp "$OUT" com.pocketagent.mobile.TaskGenerationTest
...
java -cp "$OUT" com.pocketagent.mobile.ActivityStartupTest "$PROJECT_DIR"

# Legacy Java fixtures use the app sources and small Android stubs beside them. Native
# workspace fixtures have dedicated runners with the Android SDK, built classes and JSON
# dependency they need; compiling them in this block also pulls Android Activities into the
# stub-only source path. Keep every top-level fixture covered, routing those five explicitly.
JAVA_TESTS=()
NATIVE_TEST_RUNNERS=()
for source in "$PROJECT_DIR"/tests/*Test.java; do
  name=$(basename "$source" .java)
  case "$name" in
    TaskGenerationTest|ActivityStartupTest) continue ;;
    AgentArtifactPathsTest) NATIVE_TEST_RUNNERS+=(run-agent-artifact-tests.sh); continue ;;
    CodexSkillFilesTest) NATIVE_TEST_RUNNERS+=(run-codex-skill-tests.sh); continue ;;
    RemoteMediaTest) NATIVE_TEST_RUNNERS+=(run-remote-media-tests.sh); continue ;;
    WorkspaceMediaTest) NATIVE_TEST_RUNNERS+=(run-workspace-media-tests.sh); continue ;;
    WorkspaceViewStateTest) NATIVE_TEST_RUNNERS+=(run-workspace-view-tests.sh); continue ;;
  esac
  JAVA_TESTS+=("$source")
done
if [ ${#JAVA_TESTS[@]} -gt 0 ]; then
  ...
  for source in "${JAVA_TESTS[@]}"; do
    java -cp "$UNIT" "com.pocketagent.mobile.$(basename "$source" .java)"
  done
...
code_min=$(grep -o 'MIN_SDK = [0-9]*' "$PROJECT_DIR/app/src/com/pocketagent/mobile/DeviceCheck.java" | awk '{print $3}')
...
grep -q 'POCKETAGENT_ANDROID_TARGET_SDK' "$PROJECT_DIR/app/src/com/pocketagent/mobile/ContainerRuntime.java" \
...
for gone in pocketagent-winapp.sh pocketagent-winelayer.sh pocketagent-wineprocess.py \
            pocketagent-wineboot.py pocketagent-winqueue.sh pocketagent-rdp.sh; do
  [ ! -e "$PROJECT_DIR/app/assets/$gone" ] \
    || { echo "FAIL LinuxOnly: $gone is back"; exit 1; }
done
[ ! -e "$PROJECT_DIR/app/src/com/pocketagent/mobile/WindowsApps.java" ] \
...
java_name=$(grep -m1 'static final String VERSION = ' "$PROJECT_DIR/app/src/com/pocketagent/mobile/MainActivity.java" | cut -d'"' -f2)
notes_name=$(grep -m1 -E '^# PocketAgent [0-9]' "$PROJECT_DIR/RELEASE-NOTES.md" | awk '{print $3}')
...
# Native PocketAgent beta releases carry an explicit prerelease identifier. Keep validating a
# real version instead of the old PocketAgent release cadence (which ended in 0 or 5).
[[ "$name" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-[a-z]+\.[0-9]+)?$ ]] \
  || { echo "FAIL VersionAgreement: '$name' is not a semantic release version"; exit 1; }
...
grep -q 'PromptForDownloadLocation' "$PROJECT_DIR/app/src/com/pocketagent/mobile/ContainerRuntime.java" \
  || { echo "FAIL Downloads: Chrome is not wired to the destination choice"; exit 1; }
grep -q 'LinuxApps.CHROME_POLICY' "$PROJECT_DIR/app/src/com/pocketagent/mobile/ContainerRuntime.java" \
  || { echo "FAIL Downloads: an existing Chrome policy is not repaired on desktop start"; exit 1; }
grep -q 'POCKETAGENT_DOWNLOAD_DIR' "$PROJECT_DIR/app/assets/pocketagent-desktop.sh" \
  || { echo "FAIL Downloads: the desktop does not receive its selected destination"; exit 1; }
grep -q 'Windows programs cannot run here' "$PROJECT_DIR/app/assets/pocketagent-install.sh" \
  || { echo "FAIL Installer: a downloaded Windows program must be refused with its reason"; exit 1; }
grep -q -- '--use-angle=swiftshader' "$PROJECT_DIR/app/assets/pocketagent-open.sh" \
  || { echo "FAIL Installer: ChatGPT's software renderer profile is missing"; exit 1; }
grep -q '^installed$' < <(bash "$PROJECT_DIR/app/assets/pocketagent-software.sh" --selftest) \
  || { echo "FAIL Software: the Ubuntu software centre did not self-test"; exit 1; }
grep -q 'shell monkey' "$PROJECT_DIR/app/assets/pocketagent-adb.sh" \
...
for file in "$PROJECT_DIR"/app/src/com/pocketagent/mobile/*.java "$PROJECT_DIR"/app/assets/*.sh; do
...
for helper in pocketagent-storage.sh pocketagent-software.sh pocketagent-shot.sh pocketagent-mark.png pocketagent-mcp.py pocketagent-agent.sh pocketagent-appshot.sh pocketagent-graphics.py pocketagent-appprocess.py pocketagent-childwatch.py pocketagent-adb.sh pocketagent-procinfo.py pocketagent-save.sh pocketagent-mobile.sh pocketagent-settings.sh pocketagent-files.png pocketagent-phone.png pocketagent-projects.png pocketagent-settings.png pocketagent-software.png pocketagent-package.png; do
  n=$(grep -c "$helper" "$PROJECT_DIR/app/src/com/pocketagent/mobile/ContainerRuntime.java" || true)
...
grep -q 'portraitDesktop' "$PROJECT_DIR/app/src/com/pocketagent/mobile/LinuxService.java" \
...
apps="$PROJECT_DIR/app/src/com/pocketagent/mobile/LinuxApps.java"
...
grep -q 'Name=Bin' "$PROJECT_DIR/app/assets/pocketagent-menu.sh" \
...
watch="$PROJECT_DIR/app/assets/pocketagent-desktop.sh"
helper="$PROJECT_DIR/app/assets/pocketagent-childwatch.py"
...
for gone in AndroidProcessPolicy.java ProcessPolicyOutput.java; do
  [ ! -e "$PROJECT_DIR/app/src/com/pocketagent/mobile/$gone" ] \
    || { echo "FAIL ProcessCeiling: $gone is back; the computer must manage itself"; exit 1; }
done
[ ! -e "$PROJECT_DIR/app/assets/pocketagent-process-policy.py" ] \
  || { echo "FAIL ProcessCeiling: the phone-wide policy helper is back"; exit 1; }
grep -q 'Android process limit' "$PROJECT_DIR/app/src/com/pocketagent/mobile/MainActivity.java" \
...
python3 -c "import ast,sys; ast.parse(open('$PROJECT_DIR/app/assets/pocketagent-mcp.py').read())" \
  || { echo "FAIL McpServer: pocketagent-mcp.py does not parse"; exit 1; }
mcp_out=$(printf '%s\n' \
  '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}' \
  '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' \
  | python3 "$PROJECT_DIR/app/assets/pocketagent-mcp.py")
...
echo '{"jsonrpc":"2.0","id":3,"method":"nonsense"}' \
  | python3 "$PROJECT_DIR/app/assets/pocketagent-mcp.py" | grep -q '"error"' \
...
no_device=$(printf '%s\n' \
  ...
  | env PATH=/nonexistent "$(command -v python3)" "$PROJECT_DIR/app/assets/pocketagent-mcp.py" 2>/dev/null || true)
...
echo "PASS McpServer ($(python3 "$PROJECT_DIR/app/assets/pocketagent-mcp.py" --selftest | grep -o '"' | wc -l | awk '{print $1/2}') tools)"

# No coverage is skipped when native prerequisites are unavailable: the full suite fails with
# setup instructions. Run build.sh before this runner; native suites use isolated temp dirs.
if [ ${#NATIVE_TEST_RUNNERS[@]} -gt 0 ]; then
  SDK_ROOT="${ANDROID_SDK_ROOT:-$PROJECT_DIR/../.tooling/android-sdk}"
  if [[ ! -f "$SDK_ROOT/platforms/android-35/android.jar" \
        || ! -f "$PROJECT_DIR/build/classes/com/pocketagent/mobile/DeskActivity.class" \
        || ! -f "${POCKETAGENT_JSON_JAR:-}" ]]; then
    echo 'FAIL NativeTestPrerequisites: build PocketAgent first, then set ANDROID_SDK_ROOT and POCKETAGENT_JSON_JAR (JSON-Java 20250517).' >&2
    exit 2
  fi
  for runner in "${NATIVE_TEST_RUNNERS[@]}"; do
    bash "$PROJECT_DIR/tests/$runner"
  done
  echo "PASS NativeJavaTests (${#NATIVE_TEST_RUNNERS[@]} suites)"
fi
