...
OUT="$(mktemp -d "${TMPDIR:-/tmp}/pocketagent-storage-fixtures.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
...
"${JAVAC[@]}" -encoding UTF-8 -source 8 -target 8 -cp "$JSON_JAR" -d "$OUT" \
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/RuntimeStoragePaths.java" \
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/CodexEnvironment.java" \
  ...
  "$PROJECT_DIR/tests/storage-privacy/StoragePrivacyTest.java"
java -cp "$OUT:$JSON_JAR" com.pocketagent.mobile.StoragePrivacyTest "$PROJECT_DIR"
