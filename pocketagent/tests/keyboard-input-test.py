...
SOURCE = PROJECT / 'app/src/com/pocketagent/mobile/KeyboardInputView.java'
# The dead-key table lives beside it and needs no Android of its own, so it compiles here too.
DEAD_KEYS = PROJECT / 'app/src/com/pocketagent/mobile/DeadKeys.java'
...
public boolean performEditorAction(int action){return false;} public boolean sendKeyEvent(android.view.KeyEvent event){return false;}
    ...
    'com/pocketagent/mobile/KeyboardInputHarness.java': r'''package com.pocketagent.mobile;
...
class KeyboardInputTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.work = tempfile.TemporaryDirectory(prefix='pocketagent-keyboard-test-')
    ...
    def run_case(self, name):
        result = subprocess.run(['java', '-cp', str(self.root / 'classes'), 'com.pocketagent.mobile.KeyboardInputHarness', name],
