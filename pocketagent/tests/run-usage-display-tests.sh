...
if [[ ! -f "$JSON_JAR" ]] || [[ "$(sha256sum "$JSON_JAR" | cut -d' ' -f1)" != "$EXPECTED_JSON_SHA256" ]]; then
  echo 'Set POCKETAGENT_JSON_JAR to the pinned JSON-Java 20250517 fixture dependency.' >&2
  exit 2
fi
OUT="$(mktemp -d "${TMPDIR:-/tmp}/pocketagent-usage-fixtures.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
...
"${JAVAC[@]}" -encoding UTF-8 -source 8 -target 8 -cp "$JSON_JAR" -d "$OUT" \
  ...
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/RateLimitState.java" \
  "$PROJECT_DIR/tests/agent-protocol/UsageDisplayTest.java"
java -cp "$OUT:$JSON_JAR" com.pocketagent.mobile.UsageDisplayTest
