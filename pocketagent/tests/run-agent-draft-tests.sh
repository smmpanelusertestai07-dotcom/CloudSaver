...
OUT="$(mktemp -d "${TMPDIR:-/tmp}/pocketagent-draft-fixtures.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
...
"${JAVAC[@]}" -encoding UTF-8 -source 8 -target 8 -cp "$JSON_JAR" -d "$OUT" \
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/AgentDraftState.java" \
  "$PROJECT_DIR/tests/agent-draft/AgentDraftStateTest.java"
java -cp "$OUT:$JSON_JAR" com.pocketagent.mobile.AgentDraftStateTest
