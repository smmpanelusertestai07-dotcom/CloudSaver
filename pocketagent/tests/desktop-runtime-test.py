...
DESKTOP = (PROJECT / 'app/assets/pocketagent-desktop.sh').read_text()
...
'''
    ...
    def test_display_start_precedes_menu_without_automatic_cli_registration(self):
        ...
        self.assertLess(display, DESKTOP.index('/usr/local/bin/pocketagent-menu || true'))
        # MCP connections are now explicit user actions in the Connections screen.
        # Startup must not silently grant an agent desktop/phone control tools.
        commands = '\n'.join(line for line in DESKTOP.splitlines()
                             if not line.lstrip().startswith('#'))
        self.assertNotRegex(commands, r'\b(?:claude|codex)\s+mcp\s+add\b')
