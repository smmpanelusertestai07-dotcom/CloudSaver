...
OUT="$(mktemp -d "${TMPDIR:-/tmp}/pocketagent-multi-agent.XXXXXX")"
trap 'rm -rf "$OUT"' EXIT
...
"${JAVAC[@]}" -encoding UTF-8 -source 8 -target 8 -cp "$JSON_JAR" -d "$OUT" \
  ...
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/AgentWorkspaceState.java" \
  "$PROJECT_DIR/app/src/com/pocketagent/mobile/AgentSwitchPolicy.java" \
  "$PROJECT_DIR/tests/multi-agent/AgentSwitchPolicyTest.java"
java -cp "$OUT:$JSON_JAR" com.pocketagent.mobile.AgentSwitchPolicyTest
