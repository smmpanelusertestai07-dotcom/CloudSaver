...
PRODUCTION = PROJECT / 'app/src/com/pocketagent/mobile/ProotProcess.java'
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (forced) {
            ...
            if (args[4].equals("forced-prune-history")) {
                ...
                Class<?> identityClass = Class.forName("com.pocketagent.mobile.ProotProcess$Identity");
...
class ProotProcessBehavior(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.compiler_dir = tempfile.TemporaryDirectory(prefix='pocketagent-stop-tests-')
    ...
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='pocketagent-stop-tree-')
    ...
    def run_stop(self, mode, action='sync'):
        run = subprocess.run(['java', '-cp', str(self.classes), 'com.pocketagent.mobile.ProotProcessHarness',
