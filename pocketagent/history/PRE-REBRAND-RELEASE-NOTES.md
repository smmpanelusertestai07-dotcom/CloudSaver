> Historical archive, preserved from before the PocketAgent rebrand. Names, paths, permissions, signing practices and feature claims describe older releases, not the current app. Use [the current guide](../NATIVE-BETA.md) for installation and supported behavior.

# PocketDesk 14.0.0-beta.11 — shared projects and file downloads

A project can use Codex, Cursor and Claude Code sequentially. The **Agents** sheet is
available from the side panel, the header agent name and **Model → Switch agent**.
Choosing an agent changes the official CLI harness, account, tool capabilities and chat;
the model sheet contains only that agent's advertised models. Project files are shared,
while drafts, attachments, selected model, modes, usage and sessions remain scoped to the
agent/project. Chat history is not automatically sent to another provider. Cursor offering
a model from another AI company still uses Cursor's harness and account allowance.

Only one agent connection runs at a time. Switching waits for tasks, approvals, Voice,
setup and other mutating operations to finish. Successful connection records are retained
per agent/project; returning can reconnect an installed engine with its previously selected
access level. Switching does not sign out or uninstall the other agents. Saved connections
are labelled **Previously connected**, not live account verification. Expired/revoked login
may still require the provider's sign-in flow. Antigravity remains unavailable in this build.
Project-scoped preferences are not a filesystem security boundary: the engines still share
PocketDesk's private Ubuntu environment and its existing permission model.

Chat file links now open actual APKs, ZIPs, code and other files from the selected project
or permitted private Ubuntu output folders, including /tmp, /var/tmp, /mnt/data, /workspace,
Downloads and Shared. Relative, ./relative, sandbox: and file: links are supported. Opening
creates a durable private snapshot with **Save copy** and **Share**; temporary-source deletion
does not remove an already-saved copy. The preview does not install APKs automatically.
Known HTTPS file links download on tap, and ordinary HTTPS links offer **Download file**
as an explicit action. Filename and APK MIME detection are preserved.

File copies stream to disk, up to 512 MiB with free-space checks and the existing 1 GiB
private saved-media budget. Automatic image previews keep their separate 20 MiB limit.
Traversal, symlinks, nonregular files and known credential paths are rejected. Missing files
need to be created again or supplied with a working link; a chat label cannot create bytes.
Broad phone-storage access is not added; exporting uses Android's destination picker.

Validation: focused switching, draft, artifact and media regressions, Android API 35 build,
APK signature/alignment and source archive checks. Real-phone multi-provider login, process
switching, file-save and audio testing remains outstanding. This is a development beta.

---

# PocketDesk 14.0.0-beta.10 — inline voice typing and richer chat

The microphone now sits immediately left of the Voice/Send button. Dictate records inside
the composer, with a real audio-level waveform, partial words, Stop and Cancel. It uses
Android's on-device recognizer on Android 12+ where a supported speech service/language is
installed. Microphone permission is required. No separate dictation page or silent online
recognizer fallback opens. Final words enter an unchanged draft at its saved cursor; edited
or interrupted drafts require review. Dictation never sends a prompt automatically.

Usage is visible in the composer and removed from the chat overflow menu. Actual low or
exhausted quota produces a warning with the provider's reset time and a usage-details sheet.
Stale values remain identified; neither time passing nor UI actions invent restored quota.
The warning collapses while typing, and the composer has a bounded height with attachments.

Chat-row Rename, Archive, Restore, Delete and Share now use a sheet in the current chat.
Operations still go through the official engine and verify account/project scope and the
matching completion receipt. Only explicitly sharing or saving opens Android's destination
chooser. Advanced workspace/tools screens remain accessible. Back, close and send use icons
with accessibility labels, and destructive chat actions retain confirmation.

Completed responses include relative time when a real timestamp exists, Copy text/Copy
Markdown, Share and Read aloud. User messages also have copy/share actions. Read aloud uses
an installed offline Android voice with Auto, English or Hindi selection; voice selection
changes pronunciation, not the message language. It does not translate or call a Codex TTS
API. Long text is queued in safe chunks; app pause, chat/account change and Stop cancel audio.
This is separate from the existing official Codex realtime Voice conversation.

Native chat formatting supports headings, lists, emphasis, quotes and fenced code. Code
cards show their language and separate Copy/Save controls; copying and export preserve exact
code text. Save suggests a safe .java/.py/.js/.ts/.kt/.txt or other known language extension.
Raw HTML is not executed. Actual project files still use native file/media previews.

Images from explicit image links and genuine tool output preview automatically when visible,
using existing private cached copies first. Validated public HTTPS image downloads have
bounded workers, byte/decoded-image limits and DNS/redirect checks; no account cookie, token
or referrer is forwarded. Remote hosts see the normal image request. Ordinary web-page and
video links do not silently download. Attachment cards show thumbnails/file labels, a large
preview on tap and an individual remove icon. Removing a draft reference keeps its imported
private project copy.

Expandable Thinking cards contain only the provider's public reasoning summary. Actual web
search, command, file and MCP progress appear as activity cards. Raw/encrypted reasoning is
excluded. Completed live items and restored history use real supplied timestamps, without
inventing historical arrival times.

Host checks passed: inline draft insertion (19), Markdown/code parsing (41), speech chunking
(158), relative timestamps (20), remote media (76), chat-sheet receipts (24), activity (35),
usage warnings (10), Codex controls (69), logout (17), and eight official-schema fixtures.
Restored unfinished replies become stopped without inventing a completion time.
The combined Android 35 Java sources compile. Real-phone microphone, installed voices,
account access, keyboard/font layout and playback still need device testing.

Version **14.0.0-beta.10**, code **430**, keeps the existing PocketDesk package/signing identity.
The prior bounded transport/reconnect fixes and private-storage boundaries remain in place.
This is a development beta, not an official provider app or a complete desktop clone.
...
Visual layout follows the supplied ChatGPT/Claude screenshots and the searched Codex examples.

---

# PocketDesk 14.0.0-beta.9 — reliable transport, simpler chat

**Large agent output** no longer hits the old 2 MB disconnect path. That was a PocketDesk
incoming-message restriction, not a Codex subscription limit or a selected-file upload quota.
Frames larger than 256 KiB spool to temporary private files with serialized delivery. A 16 MiB
safety bound prevents unbounded phone memory use; a frame beyond it is drained and reported
without deliberately disconnecting the engine. Outputs larger than the supported inline
format should be actual project files. This does not remove provider limits or make storage
unlimited.

**Reconnect** now handles unexpected Codex process exits and transient initialization failures
with up to three attempts at 1, 4 and 15 seconds while an unlocked foreground client remains
eligible. Two stable minutes reset the retry budget. It reuses the installed verified engine,
saved authentication, selected project/chat and control choices. Recovery does not replay
prompts, tool approvals, reset-credit requests or browser login. Explicit Disconnect/Sign out
and authoritative account rejection stop automatic reuse. Expired credentials still need the
provider's real sign-in. Opening a chosen saved chat verifies its official thread/project; a
missing chat is not silently replaced with a new conversation.

**Chat controls** remove repeated menu entries. Mode, model and effort stay in the compact
composer; **+** contains Photos & files, **@** project files, **$** skills/apps and **/** commands.
The separate microphone beside the draft opens reviewed Dictate. Connected Codex shows a Voice
waveform only when the composer has no text or attached context. Typing or attaching something
immediately changes that action to the send arrow; an active operation shows Stop. These are
icon-only actions with accessibility labels. Voice and voice typing retain their existing
account, device and explicit permission requirements.

**Navigation** removes Files, More and the separate All chats button from the side panel.
Conversations appear in the panel for the selected project, with search, a Recent/Archived
filter, per-chat menus and in-place Load earlier chats. Returned history pages are saved
before the bounded engine list advances, so older fetched pages remain searchable offline.
Cached metadata follows the observed account/provider/project and confirmed mutations; it is
not an account credential or a complete transcript backup. Deleting an engine chat leaves
independent local transcript copies and imported files. Files, Changes, Preview,
Workspace, Usage, Background tasks, Rename, Share, chat management and Environment remain in
the chat menu. Apps & tools and Account & settings remain in the side panel. Model, effort and
mode are no longer repeated in the chat menu. This preserves the existing capabilities while
freeing more room for the conversation; it does not turn unsupported cloud features into local
CLI controls.

**Files and media stay in the app.** Selected phone files remain private project copies;
import limits are 100 MiB per file, 512 MiB per project and 200 files. New engine images and
explicitly loaded remote media are saved under private app files, with no 24-hour expiry or
automatic eviction. Saved previews can reopen without fetching the same URL again. Saved
media has a 1 GiB/3,000-record bound and leaves 32 MiB of storage free; supported image decoding
remains bounded at 20 MiB/40 megapixels. Inline image encoding fits within the 16 MiB transport
bound, approximately 12 MiB decoded. Only temporary Android Share copies keep their old
24-hour expiry. Existing beta.8 cache files migrate when still present; previously evicted
files cannot be restored locally. Clear app data and uninstall still remove private files.
Remote previews still require an explicit Load preview action and validated public HTTPS;
project/generated files must really exist before the app can display them.

Host regression checks pass for transport (43 assertions under a 128 MiB heap), private/saved
media (81), remote media (62), reconnect policy (40), engine media (16) and Claude login guards
(6), plus Codex controls (68), logout (17) and eight payloads checked against the pinned
official schema, including image-only turns. Session controls have 79 assertions and local
indexing 17. Helper and media classes compile against
Android 35. The transport fixture exercises a
4 MiB image packet and following RPC, a drained 64 MiB generated frame, strict UTF-8 and
cancellation/shutdown cleanup. These checks do not exercise a live signed-in Android device.

Version **14.0.0-beta.9**, code **429**, keeps the PocketDesk update identity. Install over the
previous PocketDesk APK to retain app data. Real ARM64 phone setup, provider authentication,
recovery under Android process/battery policy, microphone, media playback and layout testing
remain necessary; host checks are not a claim of complete desktop parity or zero risk.

---

# PocketDesk 14.0.0-beta.8 — chat first, compact controls

The supplied Claude Code mobile screenshots guide a new native layout: one slim header,
full-width conversation space, warm charcoal/off-white themes and a compact prompt composer.
A partial-width side panel leaves the chat visible behind its scrim. Recent chats, projects,
files and apps live in that panel; the three-dot menu retains model/reasoning, usage,
background tasks, changes, preview, account and environment controls. Persistent bottom
navigation and the large provider/status row are removed. The send button is an icon with
an accessibility label. PocketDesk keeps its own identity and Android typography.

**+** groups attachments, **/** commands, **@** project files and **$** enabled skills/callable
apps. Enabled skills also appear as dynamic `/skill-name` choices, and tool pickers update from
actual engine inventories. Supported commands open the native control or official Codex request;
the app does not execute arbitrary slash text as shell commands. Reference names come from
the selected project's files and the current engine catalog. Unsupported slash actions and
ambiguous explicit references require correction; unknown `$` literals remain ordinary text.
Picker and file results are checked against the current provider/project/thread/account scope.
This does not add every Codex desktop or CLI command.

**Voice typing** is now separate from **Voice conversation**. On-device recognition uses
Android's explicit on-device speech service on Android 12+ with an installed supporting
language and a granted microphone permission. After granting permission, another tap starts
capture; failure does not switch that action to online recognition. Phone dictation opens
an external speech activity without PocketDesk's microphone grant. That app owns recording
and its permission choices; offline is requested but cannot be guaranteed for that route.
No microphone-permission bypass is implemented.

The user reviews recognized text and taps **Use text** to add it to the draft. It is never
automatically sent to an agent. Local recognition stops on backgrounding, app lock and focus
interruption, with a 60-second attempt limit and stale-callback rejection. Recognition service
availability, installed languages and real phone interruption behavior still need device tests.

**Media in chat** now includes validated engine image outputs and remote file/image cards,
with native preview, audio/video Play, Save copy and Share for supported formats. Remote
cards show their host and require **Load preview** before fetching. Public-HTTPS downloads
use a bounded 24-hour app-private cache with checked DNS/TLS/redirect destinations and no
cookies, account tokens or referrers. Private/local/numeric/credentialed targets are blocked.
Simple shape-only SVG logos can preview after a bounded strict parser constructs normalized
markup for a sealed, JavaScript-disabled WebView; network, file, content and navigation access
are blocked. Unsupported SVG remains available to Save/Share. Raw SVG/web content is not
executed as a general preview. This renders actual media outputs; it does not invent
image-generation or model video support.
Android codec, remote-host compatibility and real playback still need phone validation.

**Reconnect** uses the official CLI's saved login after a successful beta.8 connection,
remembering the provider/project/access choice. It runs only from an eligible foreground
client and an already installed workspace. It never starts a new login, installs an engine,
widens access or undoes explicit Disconnect/Sign out automatically. Existing beta.7 preferences
need one successful beta.8 connection; expired credentials still require explicit sign-in.
Empty UI history can load the resumed engine thread instead of showing routing instructions.
Verified Apps/MCP/Skills catalogs now refresh automatically with bounded retries, using actual
readiness metadata; no refresh auto-installs a plugin or grants a connection permission.

Host checks pass for composer parsing/catalogs (117), dictation policy (19), remote media (62),
workspace media (60), static SVG (45), reconnect (19), engine media (16), Claude login guards (6), integrations
(97), protocol (106), recovery (34), history (18), effort (421) and delivery races. Style classes
compile against Android 35; theme XML parses and sampled muted text meets 4.5:1 contrast.
Real phone/account, microphone, codec/network and final layout testing remain outstanding.

Version **14.0.0-beta.8**, code **428**, retains the PocketDesk update identity. Existing
usage/reset credits, real Codex Voice, local workspace, GitHub, account/privacy and notification
controls remain subject to their beta.7 availability and permission boundaries.

---

# PocketDesk 14.0.0-beta.7 — cleaner chat, live usage and workspace

The native layout takes its visual structure from inspected official Codex Mac and Replit
mobile references: a quiet conversation canvas, compact user messages, one composer, short
labels and fewer stacked cards. Light, Dark and System use the shared neutral palette and
Android sans typography. PocketDesk keeps its own identity and exposes version information
in About. The interface is still awaiting real-device layout, keyboard and accessibility QA.

**Effort** is now a native slider over the selected model's actual advertised choices, with
its description and an explicit Apply action. Names reflect the engine's choice: `xhigh`
reads Extra high; the app no longer relabels every model's highest option as Max. Auto uses
the advertised default. Models and effort availability remain account/engine controlled.

**Usage** shows reported quota windows as bars, reset times, credit balance and earned reset
credits where available. **Use reset credit** requires account-bound confirmation and fresh
verified account data, then calls the official consume operation. Uncertain retries reuse a persisted,
account-scoped idempotency key. Only a returned outcome and refreshed limits affect the display;
no local clock or counter manufactures a reset. Manage credits opens the official account page.
Missing data remains unavailable and token counts remain separate from subscription quota.

Account, usage and model state now combine engine notifications, lifecycle updates and bounded
polling while eligible screens are visible/unlocked. The app pauses conflicting reads and
backs off on failures. GitHub/Git status and visible tool catalogs also update within bounded
refresh rules, with Retry for failures. This removes routine manual refresh steps without
promising instant synchronization, background uptime or compatibility with unknown future APIs.
New catalog entries can appear automatically; new protocols still need a tested app update.

**Live** opens actual on-device **Activity**, **Changes** and **Preview**: streamed engine
output, matching-thread background tasks, current files/Git changes and an interactive local
web preview. Ubuntu runs on this phone. This is not a Cursor-style cloud VM or a simulated
remote desktop video. An existing Ubuntu desktop can be opened only when it is already running;
Live does not start one automatically. Android resource limits still apply.

**Voice** is separate from **Add → Dictate text**. Voice implements the pinned Codex engine's
`thread/realtime/start`/`stop` WebRTC route using the connected ChatGPT account, with native
Start, Mute, End and returned captions. Explicit permission/consent is required. Local audio
stops when the screen is left, the app locks or audio is interrupted; an accepted coding task
can continue in Chat, where approvals still require a decision. The local transport does not
receive account tokens or save audio recordings. Dictate text only fills an editable draft.

Voice uses the installed experimental protocol capability requested by PocketDesk and requires
account/rollout/policy access plus a working Android System WebView. This is not a user-toggle
setup step; source-level method availability does not establish access for every account or plan. A rejected connection remains unavailable rather than switching to API-key
billing. No real ChatGPT account, phone microphone or physical-device WebRTC call was available
for end-to-end validation.

OpenAI documents a separate plan-dependent Voice allowance; coding tasks started through Voice
still use the Codex budget. Eligible credit-based workspaces can also spend credits on Voice.
No hardcoded allowance or unlimited-use promise is added. See
[official Voice usage rules](https://learn.chatgpt.com/docs/pricing#chatgpt-voice-in-desktop).

**Privacy & notifications** opens fixed official provider account/privacy destinations or
clearly labeled guides. PocketDesk does not pretend to read or change provider training
settings with a local Off toggle. Real on-phone Task completed, Needs approval and Agent errors
preferences control generic, deduplicated event cards where the engine reports authoritative
IDs. Android notification permission/channels still decide delivery; notifications cannot
approve a tool request. Foreground-service notices remain separate.

Version **14.0.0-beta.7**, code **427**, retains the PocketDesk application/update identity and
is supplied as APK plus source. The existing private-file import, explicit GitHub sign-in,
local provider logout, tools, skills and project controls remain. Plugin management stays
Preview under its official API limitation. Cloud workspaces, an official scheduler, Appshots,
public-chat URLs and automatic rewriting of unsupported future protocols are not implemented.
Host checks and compilation are not production, security, account or desktop-parity guarantees.

References: [Codex app](https://learn.chatgpt.com/docs/app),
[Replit mobile](https://replit.com/products/mobile),
[Codex App Server](https://learn.chatgpt.com/docs/app-server),
[pinned Codex realtime source](https://github.com/openai/codex/blob/rust-v0.154.0/codex-rs/core/src/realtime_conversation.rs),
[reviewed provider settings links](docs/ACCOUNT-SETTINGS-LINKS.md).

---

# PocketDesk 14.0.0-beta.6 — native chat, project and account controls

The native interface now uses a neutral palette with **System, Dark and Light** appearance
choices, Android sans typography and the phone's font-size setting. PocketDesk keeps its own
name and app icon. The composer groups **Mode, Model, Thinking and Usage** with labeled Add,
Mention, Actions, Mic and Send/Stop controls. `/` and `@` open native action and attachment
pickers; supported actions are mapped to controls rather than executed as arbitrary shell
commands. Android speech recognition adds editable draft text only, with consent before
use; recognition availability and processing depend on the phone's provider.

Codex controls are mapped to the pinned official **0.154.0** App Server schema. **Plan** uses
Codex's advertised built-in collaboration mode and instructions. It is available only when
the engine reports the required mode and model metadata. Ask approvals and Auto edits retain
explicit workspace-access settings; neither Plan nor PRoot is a security sandbox. Supported
native actions include compaction, code review, model/effort selection, usage and sign-out.
**Sign out of Codex** stops any current Codex task or pending login and requests official
local logout. An idle connected engine uses `account/logout`; a disconnected or interrupted
engine uses the official CLI logout command. Errors/timeouts are reported rather than treated
as success. Project files and local history stay intact; browser sessions and separate GitHub
connections are not revoked.

**Conversations** reads the engine's local project history with search, resume, rename,
archive/restore and confirmed deletion. Sharing first displays the returned user/assistant
text for review and then opens Android's share chooser; it does not create an official public
chat URL. **Tasks** displays background commands reported for the current Codex conversation,
with explicit individual/all-task stop controls. These are local running commands, not a
cloud scheduler or a promise that Android will keep them alive.

**Add** can import individually selected phone photos/files into app-private project storage.
Codex receives actual image inputs only for models that advertise image support; other files
are attached as project paths. Project media has native image preview and deliberate local
audio/video playback, plus Save copy and Share. A playable video file is not a video input
sent to Codex. No broad public-phone-folder permission or Ubuntu mount is provided.

**Project tools** adds source ZIP import, HTTPS Git clone, local Git initialization, origin
configuration, commit, fast-forward pull and non-force push with explicit review. Its native
GitHub card offers **Check GitHub setup → Install GitHub CLI → Connect GitHub account**.
Installation uses Ubuntu's signed package repository. The official `gh` browser flow returns
a one-time code for github.com; returning from the browser alone never marks the account
connected. Status checks verify the CLI's authentication and actual GitHub user identity.
Native Git uses the official credential helper for verified github.com connections; private
clone/push still requires repository permission. Other HTTPS hosts need their own Git setup.
This GitHub login is separate from the GitHub app connected through ChatGPT.

GitHub CLI owns credentials in the app-private Ubuntu profile; it can fall back to a plaintext
configuration file when a system credential store is unavailable. The native screen does not
display authentication tokens. **Sign out of GitHub locally** removes the chosen local CLI
sign-in, preserves projects and does not revoke GitHub's OAuth tokens globally. Source exports
exclude common credential files but are not a complete secret scan. GitHub browser/device-code
sign-in and private clone/push still need real phone/account testing.
[GitHub CLI login](https://cli.github.com/manual/gh_auth_login),
[Git credentials](https://cli.github.com/manual/gh_auth_setup-git),
[Local logout](https://cli.github.com/manual/gh_auth_logout).

**Environment** provides native Codex variable-name inspection and a masked value form with
explicit save confirmation. Saved values are not echoed in status or returned snapshots.
Variables belong to this phone's Codex profile and may affect other projects; commands can
read and transmit them. Reconnect and start a fresh conversation for updated configuration.
The setup helper only fills an unsent request to inspect the project; it does not execute a
setup script automatically.

The beta.5 Apps/MCP/Skills controls remain available. App readiness describes the account
runtime returned by Codex, not a guarantee that an existing chat has refreshed its tools.
Catalogs are bounded and report truncation. Plugin management remains opt-in **Preview**:
its official management methods are under development and excluded from production clients.
Incomplete plugin capability details prevent installation through the native review screen.

Version code **426** retains the PocketDesk application ID and update identity. The release
is supplied as an APK and source archive. Install over the earlier PocketDesk build to retain
app-private data; uninstalling or clearing Android app data removes the local workspace.

The inspected 159-method experimental RPC catalog for the pinned Codex release did not expose
an official scheduler, Appshots or public-chat-sharing API. PocketDesk does not simulate those
services or claim every desktop feature. Model access, plan benefits and quotas remain
provider-controlled. Host checks, schema mapping and APK compilation do not establish real
Android ARM64 login, media, hardware performance or end-to-end account workflows; those still
need phone/account testing. This remains a beta, without 100% working, security or desktop
parity guarantees.

References: [Codex App Server](https://learn.chatgpt.com/docs/app-server),
[ChatGPT appearance](https://help.openai.com/en/articles/11958281-updating-your-visual-experience-on-chatgpt),
[OpenAI brand guidelines](https://openai.com/brand/).

---

# PocketDesk 14.0.0-beta.5 — native Codex tools and connections

**Tools & connections** adds native Apps, MCP, Skills and Plugins sections for the selected
Codex project. Search and refresh use the catalogs and runtime state returned by the local
Codex engine. **Ready in Codex** means the runtime reports callable app tools; a configuration
toggle or an accessible catalog entry alone does not establish a working connection.

Apps open the official connection URL returned by Codex in the Android browser after review.
GitHub can be found and connected this way when it appears in the account's catalog and
supplies a supported connection URL. Returning from the browser does not mark it connected:
refresh checks the engine's reported state. A GitHub app connection does not automatically
configure Git command-line credentials for private clone or push.

MCP supports native HTTPS-server setup, status, OAuth browser handoff, enable/disable and
reload controls. Existing local server entries can also be inspected. Server-requested URL
flows and forms require an explicit user decision; passwords or tokens are not requested in
the server-setup form. Connecting a server can expose project content to that service through
its tools, so the form explains this before the user adds it.

Skills can be listed, enabled or disabled, created inside the selected project's
`.agents/skills` folder and selected for the next chat prompt. Creating a project skill does
not overwrite an existing one, and choosing **Use in chat** does not send the prompt itself.

Plugin management is an opt-in **Preview**. Catalog inspection and install/uninstall actions
depend on methods the official App Server documentation labels under development and excludes
from production clients. Installing a plugin does not automatically authorize its connected
services. This beta does not claim stable plugin APIs, every provider integration or full
desktop feature parity.

Version code **425** retains the PocketDesk application ID. Install as an update to retain
existing app data. Provider account eligibility, organizational policy and engine availability
still control tools and connections. No real user OAuth flow or Android-device integration
workflow has been verified end to end in the build environment. A successful APK build is not
evidence that all accounts or services will connect.
...
# PocketDesk 14.0.0-beta.4 — chat recovery and simpler controls

When Codex confirms that a saved conversation can no longer be resumed because it is missing,
PocketDesk can start a fresh conversation while preserving the locally saved messages in a
private archive. Read that archive through Account → Chat history. Archived messages are not
automatically sent into the new conversation. Other connection or authentication failures do
not silently discard a saved chat. Login credentials, project files and tool approval policies
remain unchanged by this recovery.

The main tab is now Chat (formerly Build). The compact composer uses visible Add file, Send
and Stop labels, with Model, Thinking and Usage controls grouped nearby. Technical explanations
are collapsed so the prompt and conversation have more room; relevant setup, error and approval
information remains accessible. The existing blue glass-style theme and account usage reporting
are retained.

Version code 424 retains the PocketDesk application ID. Install as an update to keep existing
data. Physical-phone authentication, chat recovery and complete agent workflows still require
device validation. This remains a beta, without a guarantee of 100% security or production
readiness.

---

# PocketDesk 14.0.0-beta.3 — blue glass interface and Codex controls

PocketDesk now uses the original PocketLinux-inspired blue palette, lightweight glass-style
native surfaces, clearer bottom navigation icons and selection states, and a matching app
logo/launcher icon. Preview controls share the same palette.

Codex reasoning effort is available beside the prompt. Choices come from the connected
model's advertised capabilities; Auto restores its advertised default and Max labels the
highest offered choice. A selection applies to the next prompt, remains specific to the
project and model, and does not change tool approval or workspace access policies.

The prompt area shows account quota reported by Codex. Its usage details include available
reset windows and conversation/request token counters. Missing quota is shown as unavailable;
token counts are not converted into an estimated remaining subscription allowance.

ZIP export now excludes additional conventional credential files and folders, including
SSH private keys, keystores, credentials.json, service-account JSON files and .aws/.kube.
This protects common file names; it does not detect every secret embedded in project source.
The export regression checks the actual ZIP and verifies that original files remain intact.
AppLock screenshot protection now also covers native dialogs from their first display.
Short-window/keyboard layouts keep Send reachable using a scrollable composer.

Version code 423 retains the existing PocketDesk application ID. Install the APK as an update
to preserve data. Existing AppLock (when enabled), official account handoff, tool approvals
and preview protections remain in place. Physical-device layout, real account login and complete agent
workflows still require validation; this remains a beta, without a 100% security or production
readiness claim.

---

# PocketDesk 14.0.0-beta.2 — account connection feedback

Account connection now opens visible setup/connection progress. The Account screen updates
its status, sign-in button, account and quota fields as events arrive, without replacing
controls during a touch. Workspace setup and project creation continue the requested
connection after their prerequisites finish. Setup/start failures are shown explicitly.

Repeated Connect taps preserve an in-progress login. The agent service publishes separate
connecting/authenticating/error states, and installer progress no longer waits behind the
long-running installation worker. The native button disables while connecting, then becomes
Continue official sign-in when the provider returns its URL.

This update uses the same application ID and signing key as beta.1 and a higher version code.
Existing PocketDesk data can be retained by installing it as an update. State-routing and
blocked-worker progress regression tests pass. Actual provider login on the user's phone
still requires device validation.

---

# PocketDesk 14.0.0-beta.1 — native mobile coding workspace

This beta adds a native Android interface for prompt-based agent tasks, touch approvals,
project files, Git actions and local web previews. A minimal Ubuntu userspace supplies the
agent workspace on the phone without starting the desktop or a VNC session. Initial setup
requires network access and free storage; Ubuntu and provider packages are downloaded when
needed rather than embedded in the small APK.

Codex and Cursor connect through their documented agent interfaces. Provider login, model
availability and account limits remain controlled by each provider. Claude Code integration
uses the unmodified official program and its own login. Antigravity account integration is
blocked pending verified provider permission; its account quota is not relayed or reused.

The new application ID is `com.pocketdesk.mobile`, so this beta installs beside PocketLinux.
The source namespace remains `com.pocketlinux`. Native libraries and APK ZIP alignment support
16 KB page sizes. This build requires Android 10 or later on an ARM64 device.

Compilation and host-side checks do not establish full device compatibility. Physical-device
setup, real provider-account login, subscription entitlements, long agent sessions and complete
build workflows have not been verified end to end. This is a beta, not a claim of production
readiness or complete desktop feature parity.
