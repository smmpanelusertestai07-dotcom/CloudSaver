...
# PocketDesk 14.0.0-beta.11 — Native mobile IDE

The launcher is now a native prompt-based coding interface over a local Ubuntu workspace. See [NATIVE-BETA.md](../NATIVE-BETA.md) for setup, supported agent connections, account limitations, build instructions and what still requires device testing.

Beta.11 adds safe switching between official coding agents on the same project, separate
per-agent drafts/settings and saved reconnections, plus working downloads of actual APKs
and other artifacts from private Ubuntu output folders. Agent and Model are separate
choices; each engine keeps its own chats, account and usage. See [RELEASE-NOTES.md](PRE-REBRAND-RELEASE-NOTES.md).

Beta.10 adds inline voice typing, composer usage, in-chat conversation sheets and richer
responses with timestamps, copy/read-aloud controls and separate code Copy/Save actions.
Image previews load automatically within guarded media limits, and attached files have
thumbnail cards with individual remove controls. Public Thinking summaries and actual tool
activity expand inside chat. Imported files and saved chat text stay in app-private storage.
This APK remains a beta. [NATIVE-BETA.md](../NATIVE-BETA.md) is the current capability and limitation guide.
The desktop documentation below is retained as historical PocketLinux 13 documentation; it
is not a description of every feature still available in this build. Beta.6 removes broad
public-phone-folder storage permissions and mounts, including in the retained subsystem.
Selected phone files are imported through the Android picker into app-private storage; the
old Phone files/public Downloads descriptions below no longer apply. The native web preview
uses a WebView, and AI services retain their own account and plan requirements.

---

## PocketLinux 13 desktop documentation
...
| Viewer | In-app RFB 3.8 client over a unix socket in app-private storage (`…/ubuntu-rootfs/home/coder/.pocketdesk/vnc.sock`, Xtigervnc `-rfbunixpath` with `-rfbport -1`), falling back to `127.0.0.1:5901` only on a container whose server has no unix support — Android shares loopback between apps, so a port here is reachable by any other app. The desktop is born the way Screen rotation says (the phone's way up on Auto-rotate) and kept the size of the screen; at 100 % it sits inside a small gap with a rounded blue border on a deep backdrop (portrait and landscape), pinch or Screen → Zoom to look closer, Fit to come back, Full screen to hide the controls |
| Controls | One bar, bottom by default (or top): Home · status · Mute · Screen ▾ (Fit, Zoom, Wider workspace, Rotate, Full screen, bar position, Bigger interface, Auto-hide, Rotation lock) · Finger/Mouse/Screen · Drag · Keyboard · Keys (Esc, Tab, Ctrl, Alt, Super, arrows, on demand) · Window ▾ (Switch, All open apps, Apps menu, Fit window, Resize, Minimise, Minimise all, Close, Force close, Reload) · Phone ▾ (Volume and mute, Microphone, Take a photo, Add a file from the phone or a cloud drive, Phone files, Paste from the phone, Lock the screen) |
...
The signing key used for delivered APKs is excluded from the shared source archive. Keep
release signing material private and stable for future updates; never include keystores or
credentials in shared source. A fresh source build creates its own development key. An APK
signed with that new key cannot update the delivered PocketDesk APK in place. For a release
of your own, configure your own private signing key through the build's signing settings.
Uninstalling the delivered app to switch signing identities removes its private workspace,
so export important source first.
...
| `RECORD_AUDIO` | **Optional and requested on use.** Phone → Microphone feeds desktop speech input; it starts only after the tap and stops as soon as the desktop screen is left |
