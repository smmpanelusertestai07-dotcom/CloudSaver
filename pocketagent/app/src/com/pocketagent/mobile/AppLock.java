...
/**
 * The app lock: the phone's own fingerprint or PIN, asked whenever PocketAgent comes back to
 * the front.
 *
...
final class AppLock {
    ...
    static View show(Activity activity, FrameLayout root, Runnable onUnlocked) {
        View existing = root.findViewWithTag("pocketagent-lock");
        if (existing != null) return existing;
        Context context = activity;
        boolean dark = true;
        LinearLayout screen = new LinearLayout(context);
        screen.setTag("pocketagent-lock");
    ...
    static boolean showing(FrameLayout root) {
        return root != null && root.findViewWithTag("pocketagent-lock") != null;
    }
        ...
        try {
            android.hardware.biometrics.BiometricPrompt.Builder builder =
                    new android.hardware.biometrics.BiometricPrompt.Builder(activity)
                            .setTitle("PocketAgent is locked")
    ...
    private static boolean credentialScreen(Activity activity, Callback callback) {
        ...
        Intent intent = keyguard.createConfirmDeviceCredentialIntent("PocketAgent is locked",
    ...
    static boolean handleResult(Activity activity, FrameLayout root, int request, int result, Runnable onUnlocked) {
        ...
        if (ok) {
            locked = false;
            View screen = root == null ? null : root.findViewWithTag("pocketagent-lock");
