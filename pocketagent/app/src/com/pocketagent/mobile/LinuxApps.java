...
/**
 * The desktop apps PocketAgent can install into the container.
 *
...
final class LinuxApps {
     ...
     * records each finished step under /var/lib/pocketagent/stage, so a set-up that was stopped
     * continues from where it stopped instead of starting over.
     */
    static final String APT_HELPERS =
            "export DEBIAN_FRONTEND=noninteractive; "
            // POCKETAGENT_TEST_ROOT is empty on the phone, so these are the real paths; the test
            // suite sets it to a temporary folder and runs these very functions.
            + "PD_ROOT=\"${POCKETAGENT_TEST_ROOT:-}\"; PD_STATE=\"$PD_ROOT/var/lib/pocketagent\"; "
            ...
            + "> \"$PD_ROOT/etc/apt/apt.conf.d/99pocketagent\"; "
            ...
            + "> \"$PD_ROOT/etc/dpkg/dpkg.cfg.d/99pocketagent\"; "
            ...
            // versions. POCKETAGENT_LIST_HOURS makes the window testable.
            ...
            + "pd_fresh() { pd_h=\"${POCKETAGENT_LIST_HOURS:-12}\"; "
            ...
            + "echo 'PocketAgent: the package list is up to date; nothing to download'; return 0; fi; "
            + "pd_u=1; while [ $pd_u -le 3 ]; do "
            + "if apt-get update 2>\"$PD_STATE/apt-update.err\"; then "
            + "date +%s > \"$PD_STATE/apt-updated-at\"; return 0; fi; "
            + "echo \"PocketAgent: package list attempt $pd_u did not finish\"; "
            ...
            + "echo \"PocketAgent: $(basename \\\"$pd_l\\\") is not answering and was set aside\"; "
            + "mv \"$pd_l\" \"$pd_l.unreachable\"; fi; done; fi; "
            + "sleep \"${POCKETAGENT_RETRY_SLEEP:-5}\"; pd_u=$((pd_u+1)); done; return 1; }; "
            ...
            + "echo \"PocketAgent: the $1 repository did not answer; removing it again\"; "
            ...
            + "|| { echo 'PocketAgent: the download stopped; it will carry on from here next time' >&2; "
            + "return 1; }; "
            + "else curl --fail --location --retry 3 --retry-delay 5 -o \"$pd_out\" \"$1\" || return 1; fi; "
            + "[ -s \"$pd_out\" ] || { echo 'PocketAgent: the download did not finish' >&2; return 1; }; "
            + "printf '%s' \"$pd_out\"; return 0; }; "
            + "pd_step() { pd_stage=$1; shift; "
            + "if [ -f \"$PD_STATE/stage/$pd_stage\" ]; then echo \"PocketAgent: $pd_stage is already done\"; return 0; fi; "
            + "pd_try=1; while [ $pd_try -le 3 ]; do "
            + "if apt-get install -y --no-install-recommends \"$@\"; then : > \"$PD_STATE/stage/$pd_stage\"; return 0; fi; "
            + "echo \"PocketAgent: $pd_stage attempt $pd_try did not finish, repairing and trying again\"; "
            + "pd_repair; pd_update || true; sleep \"${POCKETAGENT_RETRY_SLEEP:-5}\"; "
    ...
    static final String BASICS_VERSION_FILE = "var/lib/pocketagent/basics-version";
    ...
    private static String fetchKey(String url, String path) {
        return "mkdir -p /etc/apt/keyrings; curl -fsSLo /tmp/pocketagent.key '" + url + "'; "
                + "if grep -q 'BEGIN PGP' /tmp/pocketagent.key; then gpg --dearmor --yes -o '" + path
                + "' < /tmp/pocketagent.key; else cp /tmp/pocketagent.key '" + path + "'; fi; "
                + "chmod 0644 '" + path + "'; rm -f /tmp/pocketagent.key; ";
    }
    ...
    static final String CHROME_INSTALL =
            ...
            + "|| echo \"PocketAgent: Google Chrome did not finish downloading this time\"; "
    ...
    static final String TOOL_PACKAGES =
            ...
            // The words on the screen, for PocketAgent's own Appshot: an agent gets the text of a
    ...
    static final App[] CATALOG = {
            ...
            new App("creative", "Design and game tools",
                    ...
                    "pd_update || exit 11; "
                            ...
                            + "if [ ! -f /etc/profile.d/pocketagent-gl.sh ]; then "
                            + "printf 'export LIBGL_ALWAYS_SOFTWARE=1\nexport GALLIUM_DRIVER=llvmpipe\n' "
                            + "> /etc/profile.d/pocketagent-gl.sh; fi; "
                    ...
                    "apt-get remove -y --purge blender godot3 gimp inkscape >/dev/null 2>&1 || true; "
                            + "rm -f /etc/profile.d/pocketagent-gl.sh \"$PD_STATE/stage/creative\"; "
            ...
            new App("mobiledev", "Mobile app development",
                    ...
                    "pd_update || exit 11; "
                            ...
                            + "if ! grep -q 'JAVA_HOME' /etc/profile.d/pocketagent-java.sh 2>/dev/null; then "
                            + "mkdir -p /etc/profile.d; "
                            + "printf 'export JAVA_HOME=$(dirname $(dirname $(readlink -f "
                            + "$(command -v javac || command -v java))))\nexport PATH=\"$JAVA_HOME/bin:$PATH\"\n' "
                            + "> /etc/profile.d/pocketagent-java.sh; fi; "
                    ...
                    "apt-get remove -y --purge openjdk-21-jdk-headless gradle adb fastboot aapt "
                            ...
                            + "rm -f /etc/profile.d/pocketagent-java.sh \"$PD_STATE/stage/mobiledev\"; "
                            + "apt-get -y autoremove --purge >/dev/null 2>&1 || true",
                    false),
