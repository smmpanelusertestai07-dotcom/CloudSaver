...
set -u
HOME_DIR=/home/coder
STATE="$HOME_DIR/.pocketagent/adb"
mkdir -p "$STATE"
...
Wireless debugging turns itself off when the phone leaves Wi-Fi, and the port changes each time. Turn it on again and take the new port."
  ...
  install)
    ...
    [ -f "$apk" ] || { say "usage: pocketagent-adb install <file.apk>"; exit 1; }
    ...
    if [ -n "$package" ]; then
      ...
      if printf '%s' "$launch" | grep -qiE 'Events injected: 1|Monkey finished'; then
        ...
        tell "Installed" "$(basename "$apk") is installed. It has no launcher Activity PocketAgent could open automatically; start it from the phone's app list."
      fi
    else
      tell "Installed" "$(basename "$apk") is installed. PocketAgent could not read its package name, so open it from the phone's app list."
    fi
    ;;
...
Gradle: $(gradle --version 2>/dev/null | grep -i '^Gradle' | head -n 1 || echo 'not installed')"
  ...
  *)
    say "usage: pocketagent-adb {pair|connect|install <apk>|devices|logs|screen|disconnect|status}"
    exit 1 ;;
esac
