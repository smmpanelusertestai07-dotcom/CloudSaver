...
#
# Chrome already obeys the setting: PocketAgent writes it as a managed download policy at every
...
#
# Usage: pocketagent-save <file> [computer|phone|ask]
#        pocketagent-save --where          print the setting in force
#        pocketagent-save --phone-dir      print the phone folder, or say why there is none
set -u
HOME_DIR=${POCKETAGENT_HOME_DIR:-${HOME:-/home/coder}}
# The same folder Chrome is pointed at, so both halves agree without a second setting.
PHONE_DIR=${POCKETAGENT_PHONE_DOWNLOAD_DIR:-$HOME_DIR/Phone/Download/PocketAgent}
...
note() {   # note <title> <body>
  if have notify-send; then
    notify-send -a PocketAgent -i pocketagent-linux "$1" "$2" 2>/dev/null || true
  fi
  printf '%s: %s\n' "$1" "$2"
}

current() {
  case "${POCKETAGENT_DOWNLOAD_TARGET:-}" in
    computer|phone|ask) printf '%s' "$POCKETAGENT_DOWNLOAD_TARGET" ;;
    *) printf 'computer' ;;
  esac
}
...
#
# Phone files binds the phone's storage at ~/Phone. With the permission off PocketAgent leaves a
...
Turn on Phone files in PocketAgent -> Settings -> Permissions, and the phone's Download, Photos
...
case "${1:-}" in
  ...
  ""|-h|--help)
    say "usage: pocketagent-save <file> [computer|phone|ask]"
