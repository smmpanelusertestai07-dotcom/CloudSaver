...
#
# One statfs is the whole measurement. PRoot maps / to PocketAgent's own private folder on the
# phone, so the kernel answers for the phone's data partition: the same partition Android's own
# Settings screen counts, and the same free figure the PocketAgent app shows. There is no
# PocketAgent quota to report, because Android gives an app none. What this computer is itself
# using is not shown here -- the only way to get it in the container is a du over gigabytes of
# Ubuntu, minutes of disk competing with the running apps. PocketAgent -> Settings -> Storage has
...
Open PocketAgent on the phone: the home screen and Settings -> Storage always show it."
else
  advice="There is room here to install another AI app and keep working."
  if [ "$free_bytes" -lt 500000000 ]; then
    advice="Android is about to start clearing app caches by itself. Free some space now: remove an AI app you are not using from PocketAgent's Apps tab, or empty Downloads."
...
PocketAgent -> Settings -> Storage on the phone shows what this computer is using."
fi

if command -v zenity >/dev/null 2>&1; then
  zenity --info --width=430 --no-markup --title="Storage" --text="$body" >/dev/null 2>&1
elif command -v notify-send >/dev/null 2>&1; then
  notify-send -a PocketAgent -u normal "Storage" "$body" >/dev/null 2>&1
else
  printf '%s\n' "$body"
fi
