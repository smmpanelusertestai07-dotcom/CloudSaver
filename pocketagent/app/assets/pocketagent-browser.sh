...
if [ -z "$browser" ]; then
  command -v notify-send >/dev/null 2>&1 && notify-send -u critical 'Browser is not installed' 'Install Chrome from PocketAgent Apps, then retry sign-in.'
  exit 127
fi
opener=$(command -v pocketagent-open) || exit 127
...
# started on a second tap: pocketagent-open hands URLs to the existing browser singleton.
