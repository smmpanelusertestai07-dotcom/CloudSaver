...
#
# Usage: pocketagent-windows list|minimise-all|close-all|kill-active|fit|unmaximise|minimise|panel-edge|count|menu|refresh
set -u
...
case "${1:-list}" in
  ...
  menu)
    ...
    # its menu on Super+A (bound by pocketagent-menu); a synthetic key press is the one way a
  ...
  close-all)
    ...
    if [ "$count" = 0 ]; then
      command -v notify-send >/dev/null 2>&1 &&
        notify-send -a PocketAgent "Nothing to close" "No windows are open." || true
      exit 0
    fi
...
EOF
  ...
  fit)
    ...
    /usr/local/bin/pocketagent-window-guard once >/dev/null 2>&1 || true
    ;;
  unmaximise)
  ...
  minimise)
    ...
    ;;
  panel-edge)
    # Which edge the bar lives on. pocketagent-menu writes the new panel_position and tint2
    ...
    printf '%s\n' "$edge" > "$HOME/.config/pocketagent/panel-edge"
    /usr/local/bin/pocketagent-menu >/dev/null 2>&1 || true
    ...
    ( sleep 0.5; /usr/local/bin/pocketagent-window-guard once >/dev/null 2>&1 || true ) &
    ;;
  kill-active)
...
EOF
    if [ -z "$rows" ]; then
      command -v notify-send >/dev/null 2>&1 &&
        notify-send -a PocketAgent "No open windows" "Open an app from the desktop or the menu." || true
      exit 0
    fi
