...
exec python3 - "$@" <<'POCKETAGENT_GUARD_PY'
import fcntl
import os
...
STATE_DIR = Path(os.environ.get("POCKETAGENT_STATE_DIR", "/home/coder/.pocketagent"))
...
def work_area():
    override = os.environ.get("POCKETAGENT_WORKAREA", "")
...
CONFIG_DIR = Path(os.environ.get("POCKETAGENT_CONFIG_DIR", "/home/coder/.config/pocketagent"))


def chromium_scale(width, height):
    """The same arithmetic as pocketagent-desktop: short side over 560, capped by the dpi.
...
class WindowChanges:
    ...
    def __init__(self):
        self.override = valid_area(os.environ.get("POCKETAGENT_WORKAREA", ""))
