...
status() {
  ...
  line "  Expo project" "$(if [ -f package.json ] && grep -q '\"expo\"' package.json 2>/dev/null; then echo 'this folder is one'; else echo 'run: pocketagent-mobile new'; fi)"
  say ""
  say "Devices"
  if have adb; then
  ...
  say ""
  say "This phone is 127.0.0.1. Pair it once with: pocketagent-adb pair"
...
missing_android() {
  for pd_tool in java gradle adb; do
    have "$pd_tool" || { say "$pd_tool is not installed. Add 'Mobile app development' from PocketAgent's Apps tab."; return 0; }
  done
  return 1
}
...
Then, to see it running:
  ...
  in this computer — press w for the browser preview."
    ...
    android)
      ...
      # PocketAgent, which would be out of date the month after it shipped.
      ...
      ;;
    *)
      say "usage: pocketagent-mobile new [expo|android] [name]"
      exit 1 ;;
  esac
}
...
run_on_phone() {
  ...
  /usr/local/bin/pocketagent-adb install "$apk"
}
...
case "${1:-status}" in
  ...
  build)   build_android ;;
  run)     run_on_phone ;;
  devices) /usr/local/bin/pocketagent-adb devices ;;
  pair)    /usr/local/bin/pocketagent-adb pair ;;
...
1. Write it here as a React Native app through Expo:
     pocketagent-mobile new expo my-app
...
3. When you need a real iOS build for TestFlight or the App Store, Expo's build service
   ...
   Developer account — Apple's charge, not PocketAgent's.

Android needs none of that: it is built, installed and opened right here."
    ;;

  *)
    say "usage: pocketagent-mobile {status|built|new [expo|android] [name]|build|run|devices|pair|ios}"
    exit 1 ;;
esac
