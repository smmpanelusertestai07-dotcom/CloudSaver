#!/bin/bash
# PocketAgent's lightweight software centre. It is intentionally an interface to Ubuntu's signed
...
search_ubuntu() {
  ...
  run_terminal "sudo apt-get install -y --no-install-recommends '$package' && sudo /usr/local/bin/pocketagent-menu"
}
...
case "${1:-menu}" in
  ...
  update) run_terminal "sudo apt-get update && sudo apt-get -y upgrade && sudo /usr/local/bin/pocketagent-menu" ;;
  install-file) exec /usr/local/bin/pocketagent-install ;;
  installed) show_installed ;;
  --selftest) printf 'search\nupdate\ninstall-file\ninstalled\n'; exit 0 ;;
  menu)
    if ! have apt-cache || ! have apt-get; then
      tell "Software is unavailable" "This Linux computer is missing Ubuntu's package tools. Update Computer basics from PocketAgent Settings."
      exit 1
    fi
    if ! have zenity; then
      printf 'usage: pocketagent-software {search|update|install-file|installed}\n'
      exit 2
    fi
    action=$(zenity --list --radiolist --title="Software" --width=600 --height=390 \
      ...
      FALSE "Install a downloaded package" "Run PocketAgent's file safety checks" \
      FALSE "See installed software" "Names and versions" \
      --print-column=2 2>/dev/null) || exit 0
    case "$action" in
      "Find Ubuntu software") search_ubuntu ;;
      "Update installed software") run_terminal "sudo apt-get update && sudo apt-get -y upgrade && sudo /usr/local/bin/pocketagent-menu" ;;
      "Install a downloaded package") exec /usr/local/bin/pocketagent-install ;;
