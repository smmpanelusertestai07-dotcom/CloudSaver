...
#
# Usage: pocketagent-open [--label <text>] [--real-dir <folder>] [--log-name <name>]
#        [--probe-seconds <n>]
#        <command> [args...]
set -u
...
if [ -z "$label" ]; then
  # Written by pocketagent-menu for every wrapped launcher, keyed by this same name. The label
  ...
  label=$(awk -F '\t' -v k="$name" '$1 == k { print $2; exit }' "$HOME/.config/pocketagent/labels" 2>/dev/null)
fi
[ -n "$label" ] || label=$name
...
# for stdout/stderr. PocketAgent's supervisor already owns both streams. Execute
...
app_process_helper="$(dirname -- "$0")/pocketagent-appprocess.py"
...
free_mb() {
  ...
  [ -n "${POCKETAGENT_FREE_MB:-}" ] && { printf '%s' "$POCKETAGENT_FREE_MB"; return 0; }
...
notify() {   # notify <urgency> <summary> <body>
  ...
  notify-send -a PocketAgent -u "$1" -i "$name" "$2" "$3" >/dev/null 2>&1 || true
}
...
is_chromium() {
  ...
  # These are Chromium/Electron packages provided by PocketAgent's own catalogue.
  case "$name" in
...
if [ "$name" = "chatgpt" ]; then
  ...
  export BROWSER="${BROWSER:-/usr/local/bin/pocketagent-browser}"
...
chromium_scale() {
  # Worked out once by pocketagent-desktop when the desktop starts, from the geometry and dpi
  ...
  local saved
  saved=$(cat "$HOME/.config/pocketagent/chromium-scale" 2>/dev/null)
...
if [ "$managed_app" = 1 ]; then
  mkdir -p "$HOME/.pocketagent/run"
  exec 8>"$HOME/.pocketagent/run/open-$log_name.lock"
...
if [ "$status" = 0 ]; then
  if [ "$probe_seconds" -gt 0 ]; then
    ...
    stable_seconds=${POCKETAGENT_PROBE_STABLE_SECONDS:-8}
    ...
    if [ "$status" = 0 ]; then
      proof_file=${POCKETAGENT_PROOF_FILE:-}
