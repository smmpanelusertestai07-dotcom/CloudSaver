...
spec = importlib.util.spec_from_file_location('procinfo', Path(__file__).with_name('pocketagent-procinfo.py'))
procinfo = importlib.util.module_from_spec(spec)
spec.loader.exec_module(procinfo)

spec = importlib.util.spec_from_file_location('childwatch', Path(__file__).with_name('pocketagent-childwatch.py'))
...
def supervise(log_path, label, command):
    ...
    try:
        ...
        if status not in (0, 143) and shutil.which('notify-send'):
            try:
                subprocess.run(['notify-send', '-a', 'PocketAgent', '-u', 'critical',
