...
exec python3 - "$@" <<'POCKETAGENT_STATUS_PY'
import os
...
def collect(sys_root="/sys", meminfo="/proc/meminfo", storage="/"):
    ...
    try:
        ...
        # One statfs on / answers for PocketAgent's private data partition. Do
...
def render(values, output=sys.stdout, tooltip=sys.stderr):
    ...
    print(line1 or "PocketAgent", file=output)
    if line2:
