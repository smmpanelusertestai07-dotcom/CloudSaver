#!/usr/bin/env python3
"""PocketAgent's eyes and hands, offered to any AI agent over MCP.
...
So PocketAgent offers them itself, as a Model Context Protocol server over stdin and stdout. Any
...
def capture(whole_screen):
    ...
    folder = tempfile.mkdtemp(prefix="pocketagent-shot-")
...
def read_text(whole_screen):
    ...
    folder = tempfile.mkdtemp(prefix="pocketagent-ocr-")
    try:
...
# (Tools -> Phone app testing, or pocketagent-adb pair) and an agent inside this computer can
...
def adb(argv, timeout=ADB_TIMEOUT):
    ...
    if not have("adb"):
        return 127, "", ("adb is not installed. Add 'Mobile app development' from PocketAgent's "
...
def tool_phone_screenshot(_arguments):
    ...
    if missing:
        return [{"type": "text", "text": missing}], True
    folder = tempfile.mkdtemp(prefix="pocketagent-phoneshot-")
    remote = "/sdcard/pocketagent-shot.png"
...
TOOLS = [
    {
        "name": "appshot",
        "description": (
            "Look at the window in front of the PocketAgent desktop: a picture of it plus the "
    ...
    },
    {
        "name": "phone_devices",
        "description": (
            "Every phone this computer can install onto and test on. The phone PocketAgent is "
...
def handle(message):
    ...
    if method == "initialize":
        reply(identifier, {
            ...
            "instructions": (
                "This is the PocketAgent desktop: an Ubuntu computer running on the owner's "
                "phone.\n\n"
