#!/bin/bash
# PocketAgent's app installer: what happens when a Linux app package is opened inside the
...
#
# Usage: pocketagent-install [file.deb]      (no file: asks for one, starting in Downloads)
#        pocketagent-install --report <file> (prints the checks, installs nothing)
set -u
...
if [ -z "$FILE" ]; then
  if have zenity; then
    PICK_DIR=$(cat "$HOME/.config/pocketagent/download-dir" 2>/dev/null || true)
    case "$PICK_DIR" in
      "$HOME/Downloads"|"$HOME/Phone/Download/PocketAgent") ;;
    ...
    printf 'Usage: pocketagent-install <file.deb>\n'
    exit 2
  fi
fi
...
for known in $PUBLISHED; do
  ...
  warn "PocketAgent installs this app itself, from its publisher's own signed repository — the Apps tab, or Settings for Google Chrome. That copy is verified and updates in place; this downloaded one is neither."
done
...
sim_log=$(mktemp 2>/dev/null || echo /tmp/pocketagent-install-sim.log)
if [ "${POCKETAGENT_SIMULATE:-1}" = "1" ] && have apt-get && have sudo; then
...
Update the computer's basics in PocketAgent (Settings -> Storage), then open the file again."
  exit 1
fi

log="$HOME/.pocketagent/logs/install.log"
mkdir -p "$(dirname "$log")" 2>/dev/null || true

fifo=""
if have zenity; then
  fifo="/tmp/pocketagent-install-$$.progress"
  rm -f "$fifo"
  if mkfifo "$fifo" 2>/dev/null; then
    zenity --progress --pulsate --auto-close --no-cancel --width=340 \
      --title="PocketAgent" --text="Installing ${PACKAGE:-the app}..." < "$fifo" >/dev/null 2>&1 &
    exec 9<>"$fifo"
  else
    fifo=""
  fi
...
if sudo DEBIAN_FRONTEND=noninteractive apt-get install -y "$FILE" >> "$log" 2>&1; then
  ...
  sudo /usr/local/bin/pocketagent-menu >/dev/null 2>&1 || true
