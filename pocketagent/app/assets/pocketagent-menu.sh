...
#
# Every launcher points at pocketagent-open rather than the app directly, because a Chromium-based
...
OPEN=/usr/local/bin/pocketagent-open
WINDOWS=/usr/local/bin/pocketagent-windows
mkdir -p "$OPENBOX_DIR" "$TINT2_DIR" "$DESKTOP_DIR" "$LOCAL_APPS" \
         "$HOME_DIR/Projects" "$HOME_DIR/Downloads" "$HOME_DIR/Phone" \
         "$HOME_DIR/.config/pocketagent" "$HOME_DIR/.themes"

# The label of every wrapped launcher ("ChatGPT", "Chrome", "Files"), keyed by the name
# pocketagent-open derives from the command it is given. Rebuilt on every run and moved into
...
LABELS_NEW="$HOME_DIR/.config/pocketagent/labels.new"
: > "$LABELS_NEW"
...
# two destinations PocketAgent itself writes; a damaged file falls back to private Downloads.
DOWNLOAD_DIR=$(cat "$HOME_DIR/.config/pocketagent/download-dir" 2>/dev/null || true)
case "$DOWNLOAD_DIR" in
  "$HOME_DIR/Downloads"|"$HOME_DIR/Phone/Download/PocketAgent") ;;
...
PANEL_AT=bottom
[ "$(cat "$HOME_DIR/.config/pocketagent/panel-edge" 2>/dev/null)" = "top" ] && PANEL_AT=top
...
# A copy of the package's own entry with the launch command routed through pocketagent-open.
...
icon_for() {   # icon_for <base> -> icon name or empty to keep the original
  case "$1" in
    pcmanfm) printf 'pocketagent-files' ;;
    *) printf '' ;;
  esac
}

# The name pocketagent-open will derive from a command: its first word, after any `env
...
write_entry() {   # write_entry <source> <target> <label> <command>
  ...
  awk -v cmd="$4" -v label="$3" -v icon="$icon_override" '
    /^\[/ { group++ }
    group > 1 { next }
    /^Exec=/ { print "Exec=/usr/local/bin/pocketagent-open " cmd " %U"; next }
    /^Name=/ { print "Name=" label; next }
    /^Icon=/ && icon != "" { print "Icon=" icon; next }
    /^Name\[/ { next }
    /^(DBusActivatable|TryExec|Actions|X-PocketAgent)=/ { next }
    { print }
  ' "$1" > "$2"
  echo 'X-PocketAgent=1' >> "$2"
  chmod 755 "$2"
}
...
# the panel, Super+A, or Window -> Apps menu on the phone.
{
  ...
  echo '<menu id="root-menu" label="PocketAgent - Ubuntu 24.04 LTS">'
  found=0
  tool_items=""
  while read -r desktop; do
    ...
    if [ "$(field "$desktop" X-PocketAgent-Direct)" = "true" ]; then
      command=$(strip_codes "$(field "$desktop" Exec)")
...
EOF
  ...
  echo '    <item label="Screenshot"><action name="Execute"><command>/usr/local/bin/pocketagent-shot screen</command></action></item>'
  echo '    <item label="Screenshot (window in front)"><action name="Execute"><command>/usr/local/bin/pocketagent-shot window</command></action></item>'
  echo '    <item label="Storage"><action name="Execute"><command>/usr/local/bin/pocketagent-storage</command></action></item>'
  echo '    <item label="Software" icon="/usr/share/pixmaps/pocketagent-software.png"><action name="Execute"><command>/usr/local/bin/pocketagent-software</command></action></item>'
  echo '    <item label="Appshot to the AI app (Super+Space)"><action name="Execute"><command>/usr/local/bin/pocketagent-appshot</command></action></item>'
  echo '    <item label="AI computer use"><action name="Execute"><command>/usr/local/bin/pocketagent-agent status</command></action></item>'
  echo '    <separator/>'
  ...
  echo '      <item label="What can be built here"><action name="Execute"><command>lxterminal -e bash -lc "/usr/local/bin/pocketagent-mobile built; echo; read -p \"Press Enter to close \""</command></action></item>'
  echo '      <item label="What is installed and connected"><action name="Execute"><command>lxterminal -e bash -lc "/usr/local/bin/pocketagent-mobile status; echo; read -p \"Press Enter to close \""</command></action></item>'
  echo '      <item label="Start a new mobile app"><action name="Execute"><command>lxterminal -e bash -lc "/usr/local/bin/pocketagent-mobile new; echo; read -p \"Press Enter to close \""</command></action></item>'
  echo '      <item label="Building for iPhone"><action name="Execute"><command>/usr/local/bin/pocketagent-mobile ios</command></action></item>'
  echo '      <separator/>'
  echo '      <item label="Pair a phone"><action name="Execute"><command>/usr/local/bin/pocketagent-adb pair</command></action></item>'
  echo '      <item label="Connect"><action name="Execute"><command>/usr/local/bin/pocketagent-adb connect</command></action></item>'
  echo '      <item label="Install and open an APK"><action name="Execute"><command>/usr/local/bin/pocketagent-adb install</command></action></item>'
  echo '      <item label="App logs (logcat)"><action name="Execute"><command>/usr/local/bin/pocketagent-adb logs</command></action></item>'
  echo '      <item label="Mirror the phone screen"><action name="Execute"><command>/usr/local/bin/pocketagent-adb screen</command></action></item>'
  echo '      <item label="What is connected"><action name="Execute"><command>/usr/local/bin/pocketagent-adb status</command></action></item>'
  echo '    </menu>'
  echo '  </menu>' 
  echo '  <separator label="Folders"/>'
  echo '  <item label="Phone files" icon="/usr/share/pixmaps/pocketagent-phone.png"><action name="Execute"><command>pcmanfm /home/coder/Phone</command></action></item>'
  echo '  <item label="Projects" icon="/usr/share/pixmaps/pocketagent-projects.png"><action name="Execute"><command>pcmanfm /home/coder/Projects</command></action></item>'
  ...
  echo '  <item label="Empty the bin"><action name="Execute"><command>sh -c "rm -rf /home/coder/.local/share/Trash/files/* /home/coder/.local/share/Trash/files/.[!.]* /home/coder/.local/share/Trash/info/* 2>/dev/null; notify-send -a PocketAgent Bin \"The bin is empty.\""</command></action></item>'
  echo '  <item label="App reports"><action name="Execute"><command>pcmanfm /home/coder/.pocketagent/logs</command></action></item>'
  ...
  echo '  <item label="Settings" icon="/usr/share/pixmaps/pocketagent-settings.png"><action name="Execute"><command>/usr/local/bin/pocketagent-settings</command></action></item>'
  echo '  <item label="Install a downloaded app" icon="/usr/share/pixmaps/pocketagent-package.png"><action name="Execute"><command>/usr/local/bin/pocketagent-install</command></action></item>'
  ...
  echo '  <item label="Refresh app list"><action name="Execute"><command>/usr/local/bin/pocketagent-menu</command></action></item>'
...
# Only the entries this script wrote (they all carry X-PocketAgent=1). A blanket delete took
# away anything the owner had put on their own desktop, every time an app was installed.
for stale in "$DESKTOP_DIR"/*.desktop; do
  [ -f "$stale" ] || continue
  grep -q '^X-PocketAgent=1' "$stale" 2>/dev/null && rm -f "$stale"
done
find "$LOCAL_APPS" -maxdepth 1 -name 'pocketagent-*.desktop' -delete 2>/dev/null || true
...
rm -rf "$HOME_DIR/.pocketagent/windows" "$HOME_DIR/.pocketagent-wine" 2>/dev/null || true
rm -f "$DESKTOP_DIR"/pocketagent-win-*.desktop "$LOCAL_APPS"/pocketagent-win-*.desktop \
      "$DESKTOP_DIR/pocketagent-real-windows.desktop" \
      "$LOCAL_APPS/pocketagent-real-windows.desktop" 2>/dev/null || true

# The panel's first button: the apps menu, behind the Linux mascot. Hidden from the menu it
# opens, or it would list itself.
printf '[Desktop Entry]\nType=Application\nName=Apps\nComment=Every installed app\nExec=%s menu\nIcon=pocketagent-linux\nTerminal=false\nNoDisplay=true\nX-PocketAgent=1\n' \
  "$WINDOWS" > "$LOCAL_APPS/pocketagent-apps.desktop"
chmod 755 "$LOCAL_APPS/pocketagent-apps.desktop"
launcher_lines="
launcher_item_app = $LOCAL_APPS/pocketagent-apps.desktop"

add_favourite() {   # add_favourite <desktop file>
  base=$(basename "$1" .desktop)
  exec_line=$(field "$1" Exec)
  label=$(short_name "$base" "$(field "$1" Name)" | tr -d '"\\')
  wrapped="$LOCAL_APPS/pocketagent-$base.desktop"
...
printf '[Desktop Entry]\nType=Application\nName=Install a downloaded app\nComment=Check and install an ARM64 Linux app package\nExec=/usr/local/bin/pocketagent-install %%f\nIcon=pocketagent-package\nTerminal=false\nX-PocketAgent=1\nMimeType=application/vnd.debian.binary-package;application/x-deb;application/x-debian-package;\n' \
  > "$LOCAL_APPS/pocketagent-install.desktop"
chmod 755 "$LOCAL_APPS/pocketagent-install.desktop"

# A small Ubuntu software centre: search the signed apt catalogue, review a package, install it,
# update the computer, or hand a downloaded package to PocketAgent's safety checks.
printf '[Desktop Entry]\nType=Application\nName=Software\nComment=Search and install ARM64 software from Ubuntu\nExec=/usr/local/bin/pocketagent-software\nIcon=pocketagent-software\nTerminal=false\nCategories=System;PackageManager;\nX-PocketAgent=1\n' \
  > "$LOCAL_APPS/pocketagent-software.desktop"
chmod 755 "$LOCAL_APPS/pocketagent-software.desktop"
cp -f "$LOCAL_APPS/pocketagent-software.desktop" "$DESKTOP_DIR/pocketagent-software.desktop"

# The phone's own files, as a folder on the desktop and a button on the panel. Empty but for a
# note until the owner turns Phone files on in PocketAgent's Settings; then Download, DCIM and
# Documents are in it. Super+P opens it too.
printf '[Desktop Entry]\nType=Application\nName=Phone files\nComment=Your phone\047s storage, inside the computer\nExec=pcmanfm /home/coder/Phone\nIcon=pocketagent-phone\nTerminal=false\nX-PocketAgent=1\n' \
  > "$LOCAL_APPS/pocketagent-phone.desktop"
chmod 755 "$LOCAL_APPS/pocketagent-phone.desktop"
cp -f "$LOCAL_APPS/pocketagent-phone.desktop" "$DESKTOP_DIR/pocketagent-phone.desktop"
...
# pcmanfm chose. Its own entry gives it PocketAgent's folder, a name and a description.
printf '[Desktop Entry]\nType=Application\nName=Projects\nComment=Where your code and your work live\nExec=pcmanfm /home/coder/Projects\nIcon=pocketagent-projects\nTerminal=false\nX-PocketAgent=1\n' \
  > "$LOCAL_APPS/pocketagent-projects.desktop"
chmod 755 "$LOCAL_APPS/pocketagent-projects.desktop"
cp -f "$LOCAL_APPS/pocketagent-projects.desktop" "$DESKTOP_DIR/pocketagent-projects.desktop"
...
printf '[Desktop Entry]\nType=Application\nName=Settings\nComment=Theme, the bar, sound, storage and software\nExec=/usr/local/bin/pocketagent-settings\nIcon=pocketagent-settings\nTerminal=false\nCategories=Settings;System;\nX-PocketAgent=1\n' \
  > "$LOCAL_APPS/pocketagent-settings.desktop"
chmod 755 "$LOCAL_APPS/pocketagent-settings.desktop"
cp -f "$LOCAL_APPS/pocketagent-settings.desktop" "$DESKTOP_DIR/pocketagent-settings.desktop"
...
printf '[Desktop Entry]\nType=Application\nName=Bin\nComment=Deleted files, until the bin is emptied\nExec=pcmanfm /home/coder/.local/share/Trash/files\nIcon=user-trash\nTerminal=false\nX-PocketAgent=1\n' \
  > "$LOCAL_APPS/pocketagent-bin.desktop"
chmod 755 "$LOCAL_APPS/pocketagent-bin.desktop"
cp -f "$LOCAL_APPS/pocketagent-bin.desktop" "$DESKTOP_DIR/pocketagent-bin.desktop"
...
# the owner picks. A missing or odd .Xresources falls straight back to PocketAgent's own default.
...
px() { echo $(( $1 * DPI / 120 )); }   # a length that looked right at PocketAgent's default 120 dpi
...
# button, Super+A, a long press on the wallpaper) and PocketAgent's own Apps tab.
panel_lines=""
for base in ${BROWSER_BASE:-} pcmanfm lxterminal; do
  [ -n "$base" ] || continue
  [ -f "$LOCAL_APPS/pocketagent-$base.desktop" ] || continue
  panel_lines="$panel_lines
launcher_item_app = $LOCAL_APPS/pocketagent-$base.desktop"
done

panel_lines="$panel_lines
launcher_item_app = $LOCAL_APPS/pocketagent-phone.desktop
launcher_item_app = $LOCAL_APPS/pocketagent-settings.desktop"

MARK=/usr/share/pixmaps/pocketagent-mark.png
[ -f "$MARK" ] || MARK=/usr/share/pixmaps/pocketagent-linux.png

{
  ...
  echo 'panel_window_name = PocketAgent'
  echo 'font_shadow = 0'
  ...
  echo 'clock_lclick_command = /usr/local/bin/pocketagent-windows list'
  ...
  echo 'execp = new'
  echo 'execp_command = /usr/local/bin/pocketagent-status'
  ...
  echo 'execp_lclick_command = /usr/local/bin/pocketagent-storage'
  # The far corner: PocketAgent's own mark, doing a real job -- show the desktop, tap again to
  ...
  echo 'button_tooltip = PocketAgent - show the desktop'
  echo "button_padding = $(px 8) 0 0"
  echo "button_max_icon_size = $(px 26)"
  echo 'button_background_id = 0'
  echo 'button_centered = 1'
  echo 'button_lclick_command = /usr/local/bin/pocketagent-windows minimise-all'
  echo 'button_rclick_command = /usr/local/bin/pocketagent-windows list'
  printf '%s\n' "$launcher_lines"
  printf '%s\n' "$panel_lines"
} > "$TINT2_DIR/tint2rc"
...
for desktop in "$APPLICATIONS_DIR"/*.desktop "$LOCAL_APPS"/*.desktop; do
  [ -f "$desktop" ] || continue
  case "${desktop##*/}" in pocketagent-*) continue ;; esac
  ...
  write_entry "$desktop" "$LOCAL_APPS/pocketagent-$base.desktop" "$label" \
    "$(strip_codes "$(field "$desktop" Exec)")"
done
browser_handler=""
html_handler=""
if [ -n "$BROWSER_ENTRY" ]; then
  browser_handler=pocketagent-browser.desktop
  ...
  html_handler="pocketagent-$BROWSER_ENTRY"
  [ -f "$LOCAL_APPS/$html_handler" ] || html_handler=$BROWSER_ENTRY
  printf '[Desktop Entry]\nType=Application\nName=Browser\nExec=/usr/local/bin/pocketagent-browser %%U\nIcon=%s\nTerminal=false\nNoDisplay=true\nMimeType=x-scheme-handler/http;x-scheme-handler/https;\nX-PocketAgent=1\n' \
...
fi
{
  ...
  # A downloaded app package opens PocketAgent's installer, the way tapping an APK opens
  # Android's. Without this line the file does nothing at all when it is tapped.
  printf 'application/vnd.debian.binary-package=pocketagent-install.desktop\n'
  printf 'application/x-deb=pocketagent-install.desktop\n'
  printf 'application/x-debian-package=pocketagent-install.desktop\n'
  for desktop in "$APPLICATIONS_DIR"/*.desktop "$LOCAL_APPS"/*.desktop; do
    [ -f "$desktop" ] || continue
    case "${desktop##*/}" in pocketagent-*) continue ;; esac
    grep -qi '^Hidden=true' "$desktop" && continue
    ...
    [ -f "$LOCAL_APPS/pocketagent-$handler" ] && handler="pocketagent-$handler"
...
mkdir -p "$HOME_DIR/.themes/PocketAgent/openbox-3"
cat > "$HOME_DIR/.themes/PocketAgent/openbox-3/themerc" <<'THEMERC'
! PocketAgent -- the same palette as the phone app (see Ui.java).
!
...
#   - the decorations use PocketAgent's own theme, written above; an unreadable themerc costs one
...
OPENBOX_DEFAULT=${POCKETAGENT_OPENBOX_DEFAULT:-/etc/xdg/openbox/rc.xml}
...
chown coder:coder "$OPENBOX_DIR" "$TINT2_DIR" "$DESKTOP_DIR" "$LOCAL_APPS" \
  "$HOME_DIR/.config/pocketagent/labels" \
  "$HOME_DIR/.themes" "$HOME_DIR/.themes/PocketAgent" "$HOME_DIR/.themes/PocketAgent/openbox-3" \
  "$OPENBOX_DIR/menu.xml" "$OPENBOX_DIR/rc.xml" "$TINT2_DIR/tint2rc" \
  "$HOME_DIR/.themes/PocketAgent/openbox-3/themerc" \
  "$HOME_DIR/.config/user-dirs.dirs" "$HOME_DIR/.config/mimeapps.list" \
  "$LOCAL_APPS"/pocketagent-*.desktop "$DESKTOP_DIR"/*.desktop 2>/dev/null || true
...
if display_live; then
  ...
  if pgrep -x tint2 >/dev/null 2>&1; then
    ...
    DISPLAY=:1 setsid tint2 >/tmp/pocketagent-tint2.log 2>&1 &
  fi
  DISPLAY=:1 /usr/local/bin/pocketagent-window-guard once >/dev/null 2>&1 || true
fi
