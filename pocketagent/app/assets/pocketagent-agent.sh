...
# beta, so PocketAgent provides the capability itself, over the Model Context Protocol. This
# window is where the owner sees whether it is wired up, without opening a terminal.
set -u
MCP=/usr/local/bin/pocketagent-mcp
...
if [ -x "$MCP" ]; then
  ...
  report="PocketAgent gives any AI agent on this computer eyes and hands:
...
[ -f "$HOME/.codex/config.toml" ] && grep -q 'mcp_servers.pocketagent' "$HOME/.codex/config.toml" 2>/dev/null \
  && codex_state="ready"
claude_state="not set up"
[ -f "$HOME/.claude.json" ] && grep -q '"pocketagent"' "$HOME/.claude.json" 2>/dev/null \
  && claude_state="ready"
report="$report
...
case "${1:-status}" in
  ...
  print)  line "$report" ;;
  *)      line "usage: pocketagent-agent [status|print]"; exit 1 ;;
esac
