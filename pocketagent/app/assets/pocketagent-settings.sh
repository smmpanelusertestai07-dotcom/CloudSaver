...
# lock) say so and point at PocketAgent's own Settings instead of pretending otherwise: the
...
CONFIG="$HOME_DIR/.config/pocketagent"
...
This one belongs to the phone, not to the computer: PocketAgent writes it into the computer every
time the desktop starts, so a change made here would be replaced the next time you opened it.

Change it in PocketAgent -> Settings -> Data and files -> Downloads go to: Ask every time,
Computer Downloads or Phone Downloads."
}

about() {
  version=$(cat /var/lib/pocketagent/basics-version 2>/dev/null || echo unknown)
...
Desktop: Openbox and tint2, drawn by PocketAgent.
...
menu() {
  ...
  if ! have zenity; then
    printf 'usage: pocketagent-settings [theme-dark|theme-light|panel|appearance|sound|storage|software|downloads|about]\n'
    return 0
  fi
  choice=$(zenity --list --radiolist --width=660 --height=470 \
    --title="Settings" \
    --text="The computer's own settings. Phone permissions and the app lock live in PocketAgent." \
    ...
    FALSE size       "Text and icon size" "PocketAgent Settings" \
...
run() {
  case "$1" in
    ...
    size)       tell "Text and icon size" "The size of everything on this desktop is set in PocketAgent -> Settings -> Desktop text size.
...
Inside the desktop screen you can also use Screen -> Bigger interface, which takes effect at once." ;;
    ...
    storage)    start /usr/local/bin/pocketagent-storage ;;
    software)   start /usr/local/bin/pocketagent-software ;;
    downloads)  download_note ;;
    refresh)    /usr/local/bin/pocketagent-menu >/dev/null 2>&1; "$WINDOWS" refresh >/dev/null 2>&1 || true ;;
    about)      about ;;
    *)          menu ;;
  esac
}
