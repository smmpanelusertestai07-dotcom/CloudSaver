#!/bin/bash
# Starts the Linux desktop that PocketAgent shows. Called as:
#   pocketagent-desktop <width>x<height> <dpi>
set -u
...
def restore_system_config(source="/usr/local/share/pocketagent/dbus-system.conf",
...
export BROWSER=/usr/local/bin/pocketagent-browser
...
mkdir -p "$HOME/Pictures" "$HOME/.config/gtk-3.0" "$HOME/.config/gtk-4.0" "$HOME/.config/lxterminal" "$HOME/.config/tint2" \
         ...
         "$HOME/Downloads" "$HOME/Phone" "$HOME/.pocketagent/logs"
...
# still comes up -- PocketAgent's viewer tries the socket first and the port second.
mkdir -p "$HOME/.pocketagent"
chmod 700 "$HOME/.pocketagent" 2>/dev/null || true
...
rm -f "$HOME/.pocketagent/vnc.sock" "$HOME/.pocketagent/vnc.port" \
      /tmp/.X11-unix/X1 /tmp/.X1-lock 2>/dev/null || true

start_display() {   # start_display <extra args...>
  # Output stays on this script's stdout, which PocketAgent records for the session: a display
  ...
  /usr/bin/Xtigervnc :1 "$@" -SecurityTypes None -ac -AlwaysShared \
    -SendPrimary=0 -geometry "$GEOMETRY" -depth 24 -dpi "$DPI" -desktop 'PocketAgent' &
  VNC_PID=$!
}
...
start_display -rfbunixpath "$HOME/.pocketagent/vnc.sock" -rfbunixmode 0600 -rfbport -1
if ! wait_for_display 40; then
  ...
  rm -f "$HOME/.pocketagent/vnc.sock" /tmp/.X11-unix/X1 /tmp/.X11-unix/X1-lock /tmp/.X1-lock 2>/dev/null || true
  desktop_phase "Starting the fallback display"
  echo 5901 > "$HOME/.pocketagent/vnc.port"
...
printf '%d.%02d\n' $(( CHROMIUM_SCALE / 100 )) $(( CHROMIUM_SCALE % 100 )) > "$HOME/.config/pocketagent/chromium-scale"
...
# public Download/PocketAgent, so it is visible to Android's Files app and survives removing the
# Linux computer. If All files access was taken back, fall safely to ask/private.
DOWNLOAD_TARGET=${POCKETAGENT_DOWNLOAD_TARGET:-ask}
DOWNLOAD_DIR="$COMPUTER_DOWNLOADS"
DOWNLOAD_PROMPT=${POCKETAGENT_DOWNLOAD_PROMPT:-1}
PHONE_DOWNLOADS="$HOME/Phone/Download/PocketAgent"
...
export POCKETAGENT_DOWNLOAD_TARGET="$DOWNLOAD_TARGET"
export POCKETAGENT_DOWNLOAD_DIR="$DOWNLOAD_DIR"
export POCKETAGENT_DOWNLOAD_PROMPT="$DOWNLOAD_PROMPT"
mkdir -p "$HOME/.config/pocketagent"
printf '%s\n' "$DOWNLOAD_DIR" > "$HOME/.config/pocketagent/download-dir"
...
DESKTOP_THEME=${POCKETAGENT_THEME:-dark}
case "$(cat "$HOME/.config/pocketagent/theme" 2>/dev/null)" in
...
# setting prefer-dark.
#
# Written only while the file is still PocketAgent's. lxappearance rewrites it without the marker,
# and from then on the owner's own choice is what starts. A file written by a PocketAgent before
...
if [ ! -f "$GTK_INI" ] \
   || head -n 1 "$GTK_INI" 2>/dev/null | grep -qx '# pocketagent-default' \
...
# name a label shows is the one pocketagent-menu chooses ("Chrome", "Files", "Terminal"); a
...
# PocketAgent's own wallpaper (a dark-blue square with Tux and the app's name -- Canonical's
...
# The window manager's settings are written by pocketagent-menu, which runs below on every
...
# pocketagent-menu below from the packages that are really installed.
...
if [ -z "${FIREFOX_PROFILE:-}" ] && command -v firefox >/dev/null 2>&1; then
  mkdir -p "$HOME/.mozilla/firefox/pocketagent.default"
  printf '[Profile0]\nName=default\nIsRelative=1\nPath=pocketagent.default\nDefault=1\n\n[General]\nStartWithLastProfile=1\nVersion=2\n' \
    > "$HOME/.mozilla/firefox/profiles.ini"
  FIREFOX_PROFILE="$HOME/.mozilla/firefox/pocketagent.default"
fi
...
if ! grep -q 'PocketAgent prompt' "$HOME/.bashrc" 2>/dev/null; then
  cat >> "$HOME/.bashrc" <<'PROMPT'

# PocketAgent prompt: the computer's name, the folder, the git branch, and a red mark when the
...
PS1='\[\e[38;5;75m\]PocketAgent\[\e[0m\]:\[\e[38;5;150m\]\w\[\e[38;5;180m\]$(pd_branch)\[\e[0m\]`pd_mark`\[\e[0m\] '
...
alias keep='tmux new -A -s pocketagent'
PROMPT
fi

# Desktop/phone control tools are not automatically registered with AI agents.
# Connecting an MCP server is an explicit action in PocketAgent's Connections screen.
# Existing user configuration is preserved; Wireless debugging remains a separate Android grant.
...
if command -v gnome-keyring-daemon >/dev/null 2>&1; then
  keyfile="$HOME/.pocketagent/keyring-key"
  if [ ! -s "$keyfile" ]; then
    mkdir -p "$HOME/.pocketagent"
...
/usr/local/bin/pocketagent-menu || true

# Sound. There is no sound card a container can reach, so PulseAudio plays into a virtual
# output called Phone, and everything written to it is streamed as plain PCM to PocketAgent's
...
if command -v pulseaudio >/dev/null 2>&1; then
  ...
  audio_call pulseaudio --daemonize=yes --exit-idle-time=-1 --disable-shm=yes >/tmp/pocketagent-pulse.log 2>&1 || true
  ...
  mkdir -p "$HOME/.pocketagent"
  chmod 700 "$HOME/.pocketagent" 2>/dev/null || true
  rm -f "$HOME/.pocketagent/audio.sock" "$HOME/.pocketagent/audio.port"
  if audio_call pactl load-module module-simple-protocol-unix rate=44100 format=s16le channels=2 \
      ...
      socket="$HOME/.pocketagent/audio.sock" >/dev/null 2>&1; then
    chmod 600 "$HOME/.pocketagent/audio.sock" 2>/dev/null || true
    echo "sound: private socket" >> /tmp/pocketagent-pulse.log
  else
    echo "sound: falling back to the local port" >> /tmp/pocketagent-pulse.log
    echo 4712 > "$HOME/.pocketagent/audio.port"
  ...
  # turns bytes written by something outside PulseAudio into a source, and PocketAgent's Android
  ...
  rm -f "$HOME/.pocketagent/mic.pipe"
  mkfifo -m 600 "$HOME/.pocketagent/mic.pipe" 2>/dev/null || true
  if [ -p "$HOME/.pocketagent/mic.pipe" ]; then
    ...
    if audio_call pactl load-module module-pipe-source source_name=phone_mic \
        ...
        file="$HOME/.pocketagent/mic.pipe" format=s16le rate=16000 channels=1 \
        >> /tmp/pocketagent-pulse.log 2>&1; then
      if audio_call pactl set-default-source phone_mic >> /tmp/pocketagent-pulse.log 2>&1; then
        echo "microphone: pipe source ready" >> /tmp/pocketagent-pulse.log
      else
        echo "microphone: source loaded, but default selection failed (exit $?)" >> /tmp/pocketagent-pulse.log
      fi
    else
      echo "microphone: pipe source setup failed (exit $?)" >> /tmp/pocketagent-pulse.log
    fi
  fi
fi
...
openbox-session >/tmp/pocketagent-openbox.log 2>&1 &
DESKTOP_CHILDREN=("$!")
...
def notify(title, body, critical=False):
    command = ["notify-send", "-a", "PocketAgent", "-i", "pocketagent-linux"]
    if critical:
        command += ["-u", "critical"]
    try:
...
def offer_download(path, children):
    ...
    Two things, and neither used to happen at all. A Linux package is offered to PocketAgent's own
    ...
    if path.suffix.lower() in (".crdownload", ".part", ".tmp", ".download") or path.name.startswith("."):
        return
    marker = Path(str(path) + ".pocketagent-seen")
    try:
    ...
    if path.suffix.lower() == ".deb":
        ...
        try:
            ...
            children.append(subprocess.Popen(["/usr/local/bin/pocketagent-install", str(path)],
    ...
    try:
        children.append(subprocess.Popen(["/usr/local/bin/pocketagent-save", str(path)],
                                         stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,
                                         stderr=subprocess.DEVNULL))
    except OSError:
...
def load_childwatch():
    path = os.environ.get("POCKETAGENT_CHILDWATCH_HELPER", "/usr/local/bin/pocketagent-childwatch.py")
...
def main(home=None, download=None, panel_delay=12, display_pid=None, inherited_pids=()):
    ...
    download = Path(os.environ["POCKETAGENT_DOWNLOAD_DIR"]) if download is None else Path(download)
    state = home / ".pocketagent"
    ...
    with childwatch.ChildWakeup() as child_events, (state / "desktop-watch.lock").open("a") as lock:
        ...
        with open("/tmp/pocketagent-tint2.log", "a", buffering=1) as panel_log:
...
/usr/local/bin/pocketagent-window-guard watch >/tmp/pocketagent-window-guard-start.log 2>&1 &
DESKTOP_CHILDREN+=("$!")
command -v dunst >/dev/null 2>&1 && dunst >/tmp/pocketagent-dunst.log 2>&1 &
DESKTOP_CHILDREN+=("$!")
pcmanfm --desktop --profile LXDE >/tmp/pocketagent-pcmanfm.log 2>&1 &
DESKTOP_CHILDREN+=("$!")
