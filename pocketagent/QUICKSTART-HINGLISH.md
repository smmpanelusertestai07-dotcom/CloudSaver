# PocketAgent — 5 minute quick start (Hinglish)

## 1. Install karo

1. `PocketAgent-v13.0.0-release.apk` open karo → *Install* → "unknown apps" ka prompt aaye to allow karo.
...
4. App ka naam ab **PocketAgent** hai. Package wahi hai, isliye yeh purane version ke upar hi
...
- **Kaala start screen gaya** — pehle second me brand ka violet, aur viewer "Starting…" card dikhata hai jab
...
- **Rotation ab sach me kaam karta hai.** Settings → Screen rotation → *Portrait* ka matlab ab
  ...
  PocketAgent me Auto-rotate chuna hai, wahi chalega.
...
- **Khud ka downloaded app install karna:** desktop me Chrome se app ka Linux ARM64 `.deb` download karo → file kholo (Chrome ke download bar se ya Downloads folder se). PocketAgent ka installer khulta hai: app ka naam, version, publisher, size aur *is phone* me kitni jagah bachi hai. Processor, space, dependencies aur "unsigned file" ka check karke **Install anyway** milta hai; jo yahan chal hi nahi sakta (amd64 build, AppImage) wo reason ke saath block hota hai. Menu me **Install a downloaded app** bhi hai.
...
pehle tootta hai. PocketAgent wo capability khud deta hai (appshot + click/type/key/scroll,
...
| --- | --- |
| Home | PocketAgent home pe wapas (computer chalta rehta hai) |
...
- Computer Downloads: `/home/coder/Downloads` — private, sirf PocketAgent ke andar.
- Phone Downloads: `/home/coder/Phone/Download/PocketAgent` — Android Files me dikhta hai; Phone files permission chahiye.
