...
#
# A new application ID still installs separately even when the certificate is unchanged.
SUFFIX=""
if [[ ! -f "$KEYSTORE" ]]; then
  ...
  keytool -genkeypair -noprompt \
    ...
    -dname "CN=PocketAgent Local Preview, O=PocketAgent, C=IN" >/dev/null 2>&1
...
SIGNED_APK="$BUILD_DIR/.pocketagent-signing.apk"
