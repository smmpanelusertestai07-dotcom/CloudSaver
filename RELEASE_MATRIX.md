...
- **479 unit tests** on the JVM, covering the pure rules and auditing the
  source for claims the code does not keep.
- **Fourteen layout rules read off the source text**, in
  ...
  the keyboard, a lazy list given no ceiling, and a button label cut to one line so that the word saying what it does is lost - the first of those is not a
...
- **105 instrumented tests across 16 classes**, run on real emulators against a
