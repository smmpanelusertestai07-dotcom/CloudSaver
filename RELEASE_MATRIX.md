...
| R05 | Space caps enforced; staging hidden; leftover files clearable; the upload folder's visibility stated honestly and never hidden with .nomedia | Done | `Storage.cleanTemp` each run, `maxExtraMb` gate in the stager, `minFreeMb` reserve in `DeletePlanner`; leftover-files card in the Free up space hub; no source writes a .nomedia file anywhere; the album fact is printed beside the folder path, in setup and in faq_a2 |
...
| R16 | Permanently refused features are absent | Done | No similar-photo detection, no blur or quality scoring, no automatic deletion of originals, no cloud recommendation, no plan or price claim, no INTERNET, no reminder-only job, no re-optimise-everything action — the only mention of any of them in the source is the comment explaining the refusal |
| R17 | Setup asks in order, cannot be finished with zero albums, and Settings holds exactly the charter's groups | Done | `OnboardingSteps` (welcome, media, albums, notifications, battery, usage, cloud, ready) with the album step required and the summary warning + one-tap correction that returns to the summary; `OptionsScreen` renders eight groups covering scope, albums, layout, cloud, speed, daily cap, keep-free, may-use, storage, quality, codec, theme, lock, alerts, pause, backup/restore; SetupFlowTest |
...
| R19 | Calculator and the measured profile are one source of truth, and never render a zero as a fact | Done | `CapacityMath` + `MediaProfile` (per preset+codec, outlier-clamped, sample gate, accuracy self-check); the Typical-estimate badge sits inside the result box until 20 photos and 20 videos are measured; `shrinkPercent` is null until measured, so Home, Quality explained and Largest files all say "waiting" instead of 0%; CalculatorTest, MediaProfileTest |
| R20 | Exactly two notification channels, and the app works with notifications denied | Done | `Notifications.createChannels` creates "Working" (low, silent, only during a run) and "Alerts" (24-hour de-duplication, deep-linked, mirrored into Activity), and deletes the legacy channel; every alert path checks `warningsNotif` and posting failures are caught |
...
- **The owner's own device pass.** Ten minutes on a real phone with a real
  ...
  covered by the 430 unit tests and the instrumented suite.
