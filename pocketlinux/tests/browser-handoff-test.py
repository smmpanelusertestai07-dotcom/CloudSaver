...
class BrowserHandoff(unittest.TestCase):
    def setUp(self):
        ...
        self.script('pocketlinux-open', f'''#!{sys.executable}
...
''')
    ...
    def test_oauth_url_is_one_argument_and_browser_launch_does_not_block(self):
        ...
        status, output, elapsed = self.run_script(ASSETS / 'pocketlinux-browser.sh', url)
        ...
        self.assertNotIn(url, (self.root / '.pocketlinux/logs/browser-handoff.log').read_text())

    def test_immediate_launcher_failure_is_reported(self):
        status, _, _ = self.run_script(ASSETS / 'pocketlinux-browser.sh', 'https://auth.openai.com/', PD_BROWSER_EXIT='75')
    ...
    def test_non_web_arguments_cannot_become_browser_flags(self):
        for uri in ('--user-data-dir=/tmp/other-profile', 'file:///private', 'chatgpt://auth?code=private'):
            status, _, _ = self.run_script(ASSETS / 'pocketlinux-browser.sh', uri)
    ...
    def test_xdg_routes_web_only_and_preserves_original_protocol_handler(self):
        ...
        text = (ASSETS / 'pocketlinux-xdg-open.sh').read_text()
        script = self.script('xdg-under-test', text.replace('/usr/local/bin/pocketlinux-browser', str(browser))
