...
class WindowGuardTests(unittest.TestCase):
    def setUp(self):
        self.guard = embedded('pocketlinux-window-guard.sh', 'POCKETLINUX_GUARD_PY')
    ...
    def test_workarea_override_and_old_container_fallback(self):
        with mock.patch.dict(os.environ, {'POCKETLINUX_WORKAREA': '10 20 700 900'}), \
                mock.patch.object(self.guard, 'query') as query:
            self.assertEqual(self.guard.work_area(), (10, 20, 700, 900))
            query.assert_not_called()
        with mock.patch.dict(os.environ, {'POCKETLINUX_WORKAREA': ''}), \
    ...
    def test_new_dialog_does_not_resize_existing_window_and_removed_dialog_is_dropped(self):
        with mock.patch.dict(os.environ, {'POCKETLINUX_WORKAREA': '0 0 720 1100'}):
            changes = self.guard.WindowChanges()
        with mock.patch.object(self.guard, 'clamp_all') as fit:
...
'''
            ...
            env = dict(os.environ, PATH=str(bindir) + ':' + os.environ['PATH'],
                       GUARD_FIXTURE=str(root), POCKETLINUX_STATE_DIR=str(state))
            env.pop('POCKETLINUX_WORKAREA', None)
            process = subprocess.Popen(['bash', str(PROJECT / 'app/assets/pocketlinux-window-guard.sh'),
...
class PhoneStatusTests(unittest.TestCase):
    def setUp(self):
        self.status = embedded('pocketlinux-status.sh', 'POCKETLINUX_STATUS_PY')
    ...
    def test_temperature_units_and_real_entrypoint(self):
        ...
        result = subprocess.run(['bash', str(PROJECT / 'app/assets/pocketlinux-status.sh')],
