...
spec = importlib.util.spec_from_file_location('appprocess', ASSETS / 'pocketlinux-appprocess.py')
...
''')
        ...
        self.env = dict(os.environ, HOME=str(self.root / 'home'),
                        ...
                        POCKETLINUX_FREE_MB='2000', PD_TEST_ROOT=str(self.root))
        self.process = None
        self.log = self.root / 'home/.pocketlinux/logs/chatgpt.log'
    ...
    def command(self, *args):
        return ['bash', str(ASSETS / 'pocketlinux-open.sh'), str(self.wrapper), *args]
    ...
    def test_repeated_tap_and_hidden_callback_preserve_primary_and_browser(self):
        ...
        self.assertEqual(self.invoke(secret, POCKETLINUX_FREE_MB='200'), 0)
    ...
    def test_low_memory_defers_only_new_heavy_launch(self):
        self.assertEqual(self.invoke(POCKETLINUX_FREE_MB='500'), 75)
    ...
    def test_low_memory_browser_launch_is_deferred_but_running_browser_receives_url(self):
        ...
        self.log = self.root / 'home/.pocketlinux/logs/google-chrome.log'
        self.assertEqual(self.invoke('https://auth.openai.com/', POCKETLINUX_FREE_MB='200'), 75)
        self.assertFalse((self.root / 'starts').exists())
        self.start()
        initial = (self.root / 'arguments').read_text().splitlines()
        ...
        self.assertEqual(self.invoke(url, POCKETLINUX_FREE_MB='200'), 0)
    ...
    def test_env_prefixed_desktop_command_receives_compatibility_flags(self):
        command = ['bash', str(ASSETS / 'pocketlinux-open.sh'), 'env',
...
''')
    ...
    def test_supervisor_redacts_auth_output_and_keeps_exit_code(self):
        ...
        result = subprocess.run(['python3', str(ASSETS / 'pocketlinux-appprocess.py'),
