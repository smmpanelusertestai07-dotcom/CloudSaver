...
# app/src relative to the working directory, so `bash pocketlinux/tests/run-tests.sh` from the
...
grep -q 'POCKETLINUX_ANDROID_TARGET_SDK' "$PROJECT_DIR/app/src/com/pocketlinux/ContainerRuntime.java" \
...
for gone in pocketlinux-winapp.sh pocketlinux-winelayer.sh pocketlinux-wineprocess.py \
            pocketlinux-wineboot.py pocketlinux-winqueue.sh pocketlinux-rdp.sh; do
...
grep -q 'POCKETLINUX_DOWNLOAD_DIR' "$PROJECT_DIR/app/assets/pocketlinux-desktop.sh" \
  || { echo "FAIL Downloads: the desktop does not receive its selected destination"; exit 1; }
grep -q 'Windows programs cannot run here' "$PROJECT_DIR/app/assets/pocketlinux-install.sh" \
  || { echo "FAIL Installer: a downloaded Windows program must be refused with its reason"; exit 1; }
grep -q -- '--use-angle=swiftshader' "$PROJECT_DIR/app/assets/pocketlinux-open.sh" \
  || { echo "FAIL Installer: ChatGPT's software renderer profile is missing"; exit 1; }
grep -q '^installed$' < <(bash "$PROJECT_DIR/app/assets/pocketlinux-software.sh" --selftest) \
  || { echo "FAIL Software: the Ubuntu software centre did not self-test"; exit 1; }
grep -q 'shell monkey' "$PROJECT_DIR/app/assets/pocketlinux-adb.sh" \
...
# The rename, and the one thing it must not cost. Every name the product shows or writes is
# "pocketlinux" now; the only survivors of the old spelling are the signing key's coordinates
# (a new alias would mean a differently signed APK, which Android refuses to install over the
# one on the phone) and the code that carries an old install over.
stale=$(git -C "$PROJECT_DIR/.." ls-files -z \
  | while IFS= read -r -d '' f; do grep -HIln 'pocketdesk\|POCKETDESK\|PocketDesk' "$PROJECT_DIR/../$f" 2>/dev/null; done \
  | grep -v '/Rename.java$\|/build.sh$\|/pocketlinux-desktop.sh$\|/run-tests.sh$\|/pocketlinux.yml$' || true)
[ -z "$stale" ] || { echo "FAIL Rename: the old name is still written in:"; echo "$stale"; exit 1; }
git -C "$PROJECT_DIR/.." ls-files | grep -q 'pocketdesk' \
  && { echo "FAIL Rename: a file is still named after the old spelling"; exit 1; }
grep -q 'ks-key-alias pocketdesk' "$PROJECT_DIR/build.sh" \
  || { echo "FAIL Rename: the signing alias must stay 'pocketdesk' or the APK stops installing over the last one"; exit 1; }
grep -q 'Rename.migrate(this)' "$PROJECT_DIR/app/src/com/pocketlinux/App.java" \
  || { echo "FAIL Rename: nothing carries an old install over, so the rename costs a fresh download"; exit 1; }
for carried in var/lib/pocketdesk home/coder/.pocketdesk home/coder/.config/pocketdesk usr/local/share/pocketdesk; do
  grep -q "\"$carried\"" "$PROJECT_DIR/app/src/com/pocketlinux/Rename.java" \
    || { echo "FAIL Rename: $carried is left behind by the migration"; exit 1; }
done
grep -q 'mcp_servers.pocketdesk' "$PROJECT_DIR/app/assets/pocketlinux-desktop.sh" \
...
desk="$PROJECT_DIR/app/assets/pocketlinux-desktop.sh"
...
grep -q 'wallpaper=%s' "$desk" && grep -q 'config/pocketlinux/wallpaper' "$desk" \
  && grep -q 'set_wallpaper()' "$PROJECT_DIR/app/assets/pocketlinux-settings.sh" \
...
grep -q 'Name=Bin' "$PROJECT_DIR/app/assets/pocketlinux-menu.sh" \
...
watch="$PROJECT_DIR/app/assets/pocketlinux-desktop.sh"
helper="$PROJECT_DIR/app/assets/pocketlinux-childwatch.py"
...
[ ! -e "$PROJECT_DIR/app/assets/pocketlinux-process-policy.py" ] \
...
python3 -c "import ast,sys; ast.parse(open('$PROJECT_DIR/app/assets/pocketlinux-mcp.py').read())" \
  || { echo "FAIL McpServer: pocketlinux-mcp.py does not parse"; exit 1; }
mcp_out=$(printf '%s\n' \
  '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}' \
  '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' \
  | python3 "$PROJECT_DIR/app/assets/pocketlinux-mcp.py")
...
echo '{"jsonrpc":"2.0","id":3,"method":"nonsense"}' \
  | python3 "$PROJECT_DIR/app/assets/pocketlinux-mcp.py" | grep -q '"error"' \
...
no_device=$(printf '%s\n' \
  ...
  | env PATH=/nonexistent "$(command -v python3)" "$PROJECT_DIR/app/assets/pocketlinux-mcp.py" 2>/dev/null || true)
...
echo "PASS McpServer ($(python3 "$PROJECT_DIR/app/assets/pocketlinux-mcp.py" --selftest | grep -o '"' | wc -l | awk '{print $1/2}') tools)"
