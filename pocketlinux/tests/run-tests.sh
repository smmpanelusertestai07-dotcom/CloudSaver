...
for pd_action in search update install-file installed; do
  grep -q "^  $pd_action)" "$PROJECT_DIR/app/assets/pocketlinux-software.sh" \
    || { echo "FAIL Software: the centre cannot $pd_action"; exit 1; }
done
grep -q -- '--selftest' "$PROJECT_DIR/app/assets/pocketlinux-software.sh" \
  && { echo "FAIL Software: a test mode is reachable in the shipped software centre"; exit 1; }
grep -q 'apt-get update' "$PROJECT_DIR/app/assets/pocketlinux-software.sh" \
