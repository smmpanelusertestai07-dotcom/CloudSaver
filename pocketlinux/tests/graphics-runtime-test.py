...
spec = importlib.util.spec_from_file_location("graphics", ASSETS / "pocketlinux-graphics.py")
...
''',
        ...
        for name, script in scripts.items():
            ...
            path.chmod(0o755)
        cache = self.root / "var/cache/pocketlinux/windows"
        ...
        prefix = self.root / "home/coder/.pocketlinux/windows/old/.prefix/user.reg"
        prefix.parent.mkdir(parents=True)
        prefix.write_bytes(b"previous app sign-in")
        return dict(os.environ, PATH=str(fakebin) + ":" + os.environ["PATH"],
                    POCKETLINUX_TEST_ROOT=str(self.root), PD_STATE=str(self.root / "state"),
