...
KEYSTORE="${POCKETLINUX_KEYSTORE:-$PROJECT_DIR/.signing/pocketlinux-local.jks}"
# The alias and the store password inside the committed keystore are frozen at their original
# "pocketdesk" spelling. They are not names anyone sees; they are the coordinates of the key
# that signed every release so far, and renaming them here would either fail to open the file
# or mint a second key -- and an APK signed with a second key cannot be installed over the one
# already on the phone. The product is PocketLinux; the key keeps the name it was born with.
STORE_PASS="${POCKETLINUX_STORE_PASS:-pocketdesk-local}"
KEY_PASS="${POCKETLINUX_KEY_PASS:-$STORE_PASS}"
...
if [[ ! -f "$KEYSTORE" ]]; then
  ...
  echo "         (pocketlinux/.signing/) or set POCKETLINUX_KEYSTORE before a real release." >&2
fi
if [[ ! -f "$KEYSTORE" ]]; then
...
SIGNED_APK="$BUILD_DIR/.pocketlinux-signing.apk"
