...
(`~/.config/pocketlinux/labels`), keyed by the name the launcher derives from the command itself, and
...
Remove the old PocketLinux afterwards to get its storage back.
...
- **Files an AI app writes now obey the download setting too.** Chrome already did, through a
  managed policy. `pocketlinux-save` applies the same three answers — this computer, the phone as
...
published ChatGPT/Codex (or another verified app), but `pocketlinux-menu` then treated every
`pocketlinux-*.desktop` file as a disposable generated wrapper and deleted the permanent
`pocketlinux-win-*.desktop` entry. The verified program folder and isolated prefix survived, which
...
at every desktop start, and any other agent can use it with `python3 /usr/local/bin/pocketlinux-mcp`.
...
- **CI for PocketLinux.** A GitHub Actions workflow (`.github/workflows/pocketlinux.yml`) runs
  the seven test suites on every push that touches `pocketlinux/`, then builds the release APK
  with the Android SDK and publishes it as a downloadable artifact. It signs with the
  repository's key when the `POCKETLINUX_KEYSTORE_B64` / `POCKETLINUX_STORE_PASS` /
  `POCKETLINUX_KEY_PASS` secrets exist, otherwise with a key made for that run (such an APK
...
**Every launcher now goes through `pocketlinux-open`**, a small script that:
...
- writes everything the app printed to `~/.pocketlinux/logs/<app>.log`, and if the app dies within
