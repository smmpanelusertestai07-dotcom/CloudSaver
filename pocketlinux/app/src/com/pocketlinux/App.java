...
public final class App extends Application {
    ...
    @Override public void onCreate() {
        super.onCreate();
        Crash.install(this);
        // Before anything reads a stored setting or looks for the container: an install
        // made under the old name is carried over, once, so the rename costs nobody a
        // second download.
        Rename.migrate(this);
    ...
    private void watchForeground() {
        registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks() {
            ...
            @Override public void onActivityStopped(Activity activity) {
                ...
                // Straight away, unless this app sent the owner out itself (the camera, the
                // file picker, a settings page, the phone's own credential screen). A lock
                // that waits is a lock that can be walked past by reopening the app quickly.
                if (AppLock.expectingReturn()) handler.postDelayed(relock, 2_000L);
                else relock.run();
            }
