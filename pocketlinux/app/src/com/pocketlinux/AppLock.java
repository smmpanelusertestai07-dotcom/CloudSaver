...
final class AppLock {
    ...
    /** Called by App when no screen of this app is in front. */
    static void relock() { locked = true; }

    /**
     * When the app itself sent the owner somewhere and expects them straight back.
     *
     * Everything else re-locks the moment the app leaves the front: a lock that waits two
     * seconds is a lock anyone can walk past by tapping the app again. But this app sends the
     * owner out on purpose -- the camera, the file picker, Android's own settings pages, the
     * phone's own credential screen -- and locking behind them would ask for a fingerprint in
     * the middle of the owner's own action. Those calls say so first, and only those.
     */
    ...
    /** True while a trip the app itself started is still open. */
    ...
    static View show(Activity activity, FrameLayout root, Runnable onUnlocked) {
        ...
        screen.setTag("pocketlinux-lock");
    ...
    static boolean showing(FrameLayout root) {
        return root != null && root.findViewWithTag("pocketlinux-lock") != null;
    }
    ...
    static boolean handleResult(Activity activity, FrameLayout root, int request, int result, Runnable onUnlocked) {
        ...
        if (ok) {
            locked = false;
            View screen = root == null ? null : root.findViewWithTag("pocketlinux-lock");
