...
final class LinuxApps {
     ...
     * records each finished step under /var/lib/pocketlinux/stage, so a set-up that was stopped
    ...
    static final String APT_HELPERS =
            ...
            // versions. POCKETLINUX_LIST_HOURS makes the window testable.
            ...
            + "pd_fresh() { pd_h=\"${POCKETLINUX_LIST_HOURS:-12}\"; "
            ...
            + "sleep \"${POCKETLINUX_RETRY_SLEEP:-5}\"; pd_u=$((pd_u+1)); done; return 1; }; "
            ...
            + "pd_repair; pd_update || true; sleep \"${POCKETLINUX_RETRY_SLEEP:-5}\"; "
    ...
    static final String BASICS_VERSION_FILE = "var/lib/pocketlinux/basics-version";
    ...
    private static String fetchKey(String url, String path) {
        return "mkdir -p /etc/apt/keyrings; curl -fsSLo /tmp/pocketlinux.key '" + url + "'; "
                + "if grep -q 'BEGIN PGP' /tmp/pocketlinux.key; then gpg --dearmor --yes -o '" + path
                + "' < /tmp/pocketlinux.key; else cp /tmp/pocketlinux.key '" + path + "'; fi; "
                + "chmod 0644 '" + path + "'; rm -f /tmp/pocketlinux.key; ";
    }
    ...
    static final App[] CATALOG = {
            ...
            new App("creative", "Design and game tools",
                    ...
                    "pd_update_for blender || exit 11; "
                            ...
                            + "if [ ! -f /etc/profile.d/pocketlinux-gl.sh ]; then "
                            + "printf 'export LIBGL_ALWAYS_SOFTWARE=1\nexport GALLIUM_DRIVER=llvmpipe\n' "
                            + "> /etc/profile.d/pocketlinux-gl.sh; fi; "
                    ...
                    "apt-get remove -y --purge blender godot3 gimp inkscape >/dev/null 2>&1 || true; "
                            + "rm -f /etc/profile.d/pocketlinux-gl.sh \"$PD_STATE/stage/creative\"; "
            ...
            new App("mobiledev", "Mobile app development",
                    ...
                    "pd_update_for adb || exit 11; "
                            ...
                            + "if ! grep -q 'JAVA_HOME' /etc/profile.d/pocketlinux-java.sh 2>/dev/null; then "
                            + "mkdir -p /etc/profile.d; "
                            + "printf 'export JAVA_HOME=$(dirname $(dirname $(readlink -f "
                            + "$(command -v javac || command -v java))))\nexport PATH=\"$JAVA_HOME/bin:$PATH\"\n' "
                            + "> /etc/profile.d/pocketlinux-java.sh; fi; "
                    ...
                    "apt-get remove -y --purge openjdk-21-jdk-headless gradle adb fastboot aapt "
                            ...
                            + "rm -f /etc/profile.d/pocketlinux-java.sh \"$PD_STATE/stage/mobiledev\"; "
                            + "apt-get -y autoremove --purge >/dev/null 2>&1 || true",
                    false),
