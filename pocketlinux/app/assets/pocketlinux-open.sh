...
if [ -z "$label" ]; then
  # Written by pocketlinux-menu for every wrapped launcher, keyed by this same name. The label
  ...
  label=$(awk -F '\t' -v k="$name" '$1 == k { print $2; exit }' "$HOME/.config/pocketlinux/labels" 2>/dev/null)
fi
[ -n "$label" ] || label=$name
...
app_process_helper="$(dirname -- "$0")/pocketlinux-appprocess.py"
...
free_mb() {
  ...
  [ -n "${POCKETLINUX_FREE_MB:-}" ] && { printf '%s' "$POCKETLINUX_FREE_MB"; return 0; }
...
if [ "$name" = "chatgpt" ]; then
  ...
  export BROWSER="${BROWSER:-/usr/local/bin/pocketlinux-browser}"
...
chromium_scale() {
  # Worked out once by pocketlinux-desktop when the desktop starts, from the geometry and dpi
  ...
  local saved
  saved=$(cat "$HOME/.config/pocketlinux/chromium-scale" 2>/dev/null)
...
if [ "$managed_app" = 1 ]; then
  mkdir -p "$HOME/.pocketlinux/run"
  exec 8>"$HOME/.pocketlinux/run/open-$log_name.lock"
