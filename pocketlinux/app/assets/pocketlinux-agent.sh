...
set -u
MCP=/usr/local/bin/pocketlinux-mcp
...
[ -f "$HOME/.codex/config.toml" ] && grep -q 'mcp_servers.pocketlinux' "$HOME/.codex/config.toml" 2>/dev/null \
  && codex_state="ready"
claude_state="not set up"
[ -f "$HOME/.claude.json" ] && grep -q '"pocketlinux"' "$HOME/.claude.json" 2>/dev/null \
  && claude_state="ready"
report="$report
...
case "${1:-status}" in
  ...
  print)  line "$report" ;;
  *)      line "usage: pocketlinux-agent [status|print]"; exit 1 ;;
esac
