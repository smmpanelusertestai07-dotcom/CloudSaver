...
Files you made with it are kept." >/dev/null 2>&1 || return 0
  run_terminal "sudo apt-get remove -y '$choice' && sudo apt-get -y autoremove && sudo /usr/local/bin/pocketlinux-menu"
}

case "${1:-menu}" in
  search) search_ubuntu ;;
  update) run_terminal "sudo apt-get update && sudo apt-get -y upgrade && sudo /usr/local/bin/pocketlinux-menu" ;;
  ...
  menu)
    ...
    if ! have zenity; then
      printf 'usage: pocketlinux-software {search|update|install-file|installed|remove}\n'
      exit 2
    fi
    ...
    case "$action" in
      ...
      "Update installed software") run_terminal "sudo apt-get update && sudo apt-get -y upgrade && sudo /usr/local/bin/pocketlinux-menu" ;;
