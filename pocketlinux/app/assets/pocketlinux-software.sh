...
# -- and the same POCKETLINUX_LIST_HOURS window, so neither side downloads again what the other
# has just fetched. The rest of what apt needs here (retries, timeouts, one request at a time,
# keeping the owner's own config files) is in /etc/apt/apt.conf.d/99pocketlinux, which those same
# helpers wrote during set-up and which every apt command inside this computer reads.
#
# POCKETLINUX_TEST_ROOT is empty on the phone, so this is the real path; the test suite points it
# at a temporary folder, the same way it does for the helpers themselves.
APT_STATE="${POCKETLINUX_TEST_ROOT:-}/var/lib/pocketlinux"
APT_STAMP="$APT_STATE/apt-updated-at"

list_is_fresh() {
  hours=${POCKETLINUX_LIST_HOURS:-12}
...
update_software() {
  ...
  run_terminal "sudo dpkg --configure -a >/dev/null 2>&1; sudo apt-get -y -f install >/dev/null 2>&1; $refresh; sudo DEBIAN_FRONTEND=noninteractive apt-get -y upgrade; sudo apt-get clean; sudo /usr/local/bin/pocketlinux-menu"
}
...
search_ubuntu() {
  ...
  run_terminal "sudo apt-get install -y --no-install-recommends '$package' && sudo /usr/local/bin/pocketlinux-menu"
}
...
case "${1:-menu}" in
  ...
  install-file) exec /usr/local/bin/pocketlinux-install ;;
  ...
  menu)
    ...
    if ! have zenity; then
      printf 'usage: pocketlinux-software {search|update|install-file|installed}\n'
      exit 2
    fi
    ...
    case "$action" in
      ...
      "Install a downloaded package") exec /usr/local/bin/pocketlinux-install ;;
