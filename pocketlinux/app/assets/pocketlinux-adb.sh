...
set -u
HOME_DIR=/home/coder
STATE="$HOME_DIR/.pocketlinux/adb"
mkdir -p "$STATE"
...
Wireless debugging turns itself off when the phone leaves Wi-Fi, and the port changes each time. Turn it on again and take the new port."
  ...
  install)
    ...
    [ -f "$apk" ] || { say "usage: pocketlinux-adb install <file.apk>"; exit 1; }
...
Gradle: $(gradle --version 2>/dev/null | grep -i '^Gradle' | head -n 1 || echo 'not installed')"
  ...
  *)
    say "usage: pocketlinux-adb {pair|connect|install <apk>|devices|logs|screen|disconnect|status}"
    exit 1 ;;
esac
