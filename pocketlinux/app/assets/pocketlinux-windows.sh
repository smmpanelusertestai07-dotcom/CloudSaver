...
#
# Usage: pocketlinux-windows list|minimise-all|close-all|kill-active|fit|unmaximise|minimise|panel-edge|count|menu|refresh
set -u
...
case "${1:-list}" in
  ...
  menu)
    ...
    # its menu on Super+A (bound by pocketlinux-menu); a synthetic key press is the one way a
...
EOF
  ...
  fit)
    ...
    /usr/local/bin/pocketlinux-window-guard once >/dev/null 2>&1 || true
    ;;
  unmaximise)
  ...
  minimise)
    ...
    ;;
  panel-edge)
    # Which edge the bar lives on. pocketlinux-menu writes the new panel_position and tint2
    ...
    printf '%s\n' "$edge" > "$HOME/.config/pocketlinux/panel-edge"
    /usr/local/bin/pocketlinux-menu >/dev/null 2>&1 || true
    ...
    ( sleep 0.5; /usr/local/bin/pocketlinux-window-guard once >/dev/null 2>&1 || true ) &
    ;;
  kill-active)
