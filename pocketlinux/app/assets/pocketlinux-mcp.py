...
def capture(whole_screen):
    ...
    folder = tempfile.mkdtemp(prefix="pocketlinux-shot-")
...
def read_text(whole_screen):
    ...
    folder = tempfile.mkdtemp(prefix="pocketlinux-ocr-")
    try:
...
# (Tools -> Phone app testing, or pocketlinux-adb pair) and an agent inside this computer can
...
def tool_phone_screenshot(_arguments):
    ...
    if missing:
        return [{"type": "text", "text": missing}], True
    folder = tempfile.mkdtemp(prefix="pocketlinux-phoneshot-")
    remote = "/sdcard/pocketlinux-shot.png"
