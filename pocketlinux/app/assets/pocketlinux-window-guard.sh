...
exec python3 - "$@" <<'POCKETLINUX_GUARD_PY'
import fcntl
import os
...
STATE_DIR = Path(os.environ.get("POCKETLINUX_STATE_DIR", "/home/coder/.pocketlinux"))
...
def work_area():
    override = os.environ.get("POCKETLINUX_WORKAREA", "")
...
CONFIG_DIR = Path(os.environ.get("POCKETLINUX_CONFIG_DIR", "/home/coder/.config/pocketlinux"))


def chromium_scale(width, height):
    """The same arithmetic as pocketlinux-desktop: short side over 560, capped by the dpi.
...
class WindowChanges:
    ...
    def __init__(self):
        self.override = valid_area(os.environ.get("POCKETLINUX_WORKAREA", ""))
