...
status() {
  ...
  line "  Expo project" "$(if [ -f package.json ] && grep -q '\"expo\"' package.json 2>/dev/null; then echo 'this folder is one'; else echo 'run: pocketlinux-mobile new'; fi)"
  say ""
  say "Devices"
  if have adb; then
  ...
  say ""
  say "This phone is 127.0.0.1. Pair it once with: pocketlinux-adb pair"
...
Then, to see it running:
  ...
  in this computer — press w for the browser preview."
    ...
    android)
      ...
      ;;
    *)
      say "usage: pocketlinux-mobile new [expo|android] [name]"
      exit 1 ;;
  esac
}
...
run_on_phone() {
  ...
  /usr/local/bin/pocketlinux-adb install "$apk"
}
...
case "${1:-status}" in
  ...
  build)   build_android ;;
  run)     run_on_phone ;;
  devices) /usr/local/bin/pocketlinux-adb devices ;;
  pair)    /usr/local/bin/pocketlinux-adb pair ;;
...
1. Write it here as a React Native app through Expo:
     pocketlinux-mobile new expo my-app
...
Android needs none of that: it is built, installed and opened right here."
  ...
  *)
    say "usage: pocketlinux-mobile {status|built|new [expo|android] [name]|build|run|devices|pair|ios}"
    exit 1 ;;
esac
