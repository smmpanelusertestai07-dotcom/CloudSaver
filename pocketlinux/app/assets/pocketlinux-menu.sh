...
EOF
  ...
  echo '  <item label="Phone files" icon="/usr/share/pixmaps/pocketlinux-phone.png"><action name="Execute"><command>pcmanfm /home/coder/Phone</command></action></item>'
  echo '  <item label="Projects" icon="/usr/share/pixmaps/pocketlinux-projects.png"><action name="Execute"><command>pcmanfm /home/coder/Projects</command></action></item>'
  ...
  echo '  <item label="App reports"><action name="Execute"><command>pcmanfm /home/coder/.pocketlinux/logs</command></action></item>'
...
for base in ${BROWSER_BASE:-} pcmanfm lxterminal; do
  [ -n "$base" ] || continue
  [ -f "$LOCAL_APPS/pocketlinux-$base.desktop" ] || continue
  panel_lines="$panel_lines
launcher_item_app = $LOCAL_APPS/pocketlinux-$base.desktop"
done

panel_lines="$panel_lines
launcher_item_app = $LOCAL_APPS/pocketlinux-phone.desktop
launcher_item_app = $LOCAL_APPS/pocketlinux-settings.desktop"
...

{
  ...
  # tooltip, and pocketlinux-status writes the full sentence there. A tap opens Storage.
  echo 'execp = new'
  ...
  echo 'execp_lclick_command = /usr/local/bin/pocketlinux-storage'
...
chown coder:coder "$OPENBOX_DIR" "$TINT2_DIR" "$DESKTOP_DIR" "$LOCAL_APPS" \
  ...
  "$LOCAL_APPS"/pocketlinux-*.desktop "$DESKTOP_DIR"/*.desktop 2>/dev/null || true
