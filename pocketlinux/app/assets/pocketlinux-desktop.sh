...
if [ ! -f "$BOOKMARKS" ]; then
  ...
  bookmarks_new="$BOOKMARKS.pocketlinux-new"
  : > "$bookmarks_new"
...
if [ ! -f "$DESKTOP_ITEMS" ]; then
  ...
  desktop_items_new="$DESKTOP_ITEMS.pocketlinux-new"
...
# now, the same way pocketlinux-menu grows the panel's: a length that looked right at
...
def notify(title, body, critical=False):
    # The full path, not the name "pocketlinux-linux". PocketLinux's icons are installed in
    ...
    icon = "/usr/share/pixmaps/pocketlinux-linux.png"
