...
#   pocketlinux-desktop <width>x<height> <dpi>
set -u
...
def restore_system_config(source="/usr/local/share/pocketlinux/dbus-system.conf",
...
export BROWSER=/usr/local/bin/pocketlinux-browser
...
mkdir -p "$HOME/Pictures" "$HOME/.config/gtk-3.0" "$HOME/.config/gtk-4.0" "$HOME/.config/lxterminal" "$HOME/.config/tint2" \
         ...
         "$HOME/Downloads" "$HOME/Phone" "$HOME/.pocketlinux/logs"
...
mkdir -p "$HOME/.pocketlinux"
chmod 700 "$HOME/.pocketlinux" 2>/dev/null || true
...
rm -f "$HOME/.pocketlinux/vnc.sock" "$HOME/.pocketlinux/vnc.port" \
...
start_display -rfbunixpath "$HOME/.pocketlinux/vnc.sock" -rfbunixmode 0600 -rfbport -1
if ! wait_for_display 40; then
  ...
  rm -f "$HOME/.pocketlinux/vnc.sock" /tmp/.X11-unix/X1 /tmp/.X11-unix/X1-lock /tmp/.X1-lock 2>/dev/null || true
  desktop_phase "Starting the fallback display"
  echo 5901 > "$HOME/.pocketlinux/vnc.port"
...
# desktop was started with, and read by pocketlinux-open at every launch. A Chromium app scales
...
printf '%d.%02d\n' $(( CHROMIUM_SCALE / 100 )) $(( CHROMIUM_SCALE % 100 )) > "$HOME/.config/pocketlinux/chromium-scale"
...
DOWNLOAD_TARGET=${POCKETLINUX_DOWNLOAD_TARGET:-ask}
DOWNLOAD_DIR="$COMPUTER_DOWNLOADS"
DOWNLOAD_PROMPT=${POCKETLINUX_DOWNLOAD_PROMPT:-1}
...
export POCKETLINUX_DOWNLOAD_TARGET="$DOWNLOAD_TARGET"
export POCKETLINUX_DOWNLOAD_DIR="$DOWNLOAD_DIR"
export POCKETLINUX_DOWNLOAD_PROMPT="$DOWNLOAD_PROMPT"
mkdir -p "$HOME/.config/pocketlinux"
printf '%s\n' "$DOWNLOAD_DIR" > "$HOME/.config/pocketlinux/download-dir"
...
DESKTOP_THEME=${POCKETLINUX_THEME:-dark}
case "$(cat "$HOME/.config/pocketlinux/theme" 2>/dev/null)" in
...
if [ ! -f "$GTK_INI" ] \
   || head -n 1 "$GTK_INI" 2>/dev/null | grep -qx '# pocketlinux-default' \
...
# name a label shows is the one pocketlinux-menu chooses ("Chrome", "Files", "Terminal"); a
...
WALLPAPER=/usr/share/backgrounds/pocketlinux.jpg
CHOSEN_WALLPAPER=$(cat "$HOME/.config/pocketlinux/wallpaper" 2>/dev/null || true)
...
# The window manager's settings are written by pocketlinux-menu, which runs below on every
...
# pocketlinux-menu below from the packages that are really installed.
...
if [ -z "${FIREFOX_PROFILE:-}" ] && command -v firefox >/dev/null 2>&1; then
  mkdir -p "$HOME/.mozilla/firefox/pocketlinux.default"
  printf '[Profile0]\nName=default\nIsRelative=1\nPath=pocketlinux.default\nDefault=1\n\n[General]\nStartWithLastProfile=1\nVersion=2\n' \
    > "$HOME/.mozilla/firefox/profiles.ini"
  FIREFOX_PROFILE="$HOME/.mozilla/firefox/pocketlinux.default"
fi
...
alias keep='tmux new -A -s pocketlinux'
PROMPT
fi
...
if [ -x /usr/local/bin/pocketlinux-mcp ]; then
  # Codex reads one TOML file. Appended once, behind its own marker.
  mkdir -p "$HOME/.codex"
  # A registration left by the old name points at a program that no longer exists, and Codex
  # reports a failed server every start. Its block runs to the next section header or the end.
  if grep -q 'mcp_servers.pocketdesk' "$HOME/.codex/config.toml" 2>/dev/null; then
    awk '
      /^\[mcp_servers\.pocketdesk\]/ { drop = 1; next }
      /^\[/ { drop = 0 }
      drop { next }
      { print }
    ' "$HOME/.codex/config.toml" > "$HOME/.codex/config.toml.new" 2>/dev/null \
      && mv -f "$HOME/.codex/config.toml.new" "$HOME/.codex/config.toml"
  fi
  command -v claude >/dev/null 2>&1 \
    && grep -q '"pocketdesk"' "$HOME/.claude.json" 2>/dev/null \
    && timeout --foreground --kill-after=2s 15s claude mcp remove --scope user pocketdesk >/dev/null 2>&1 || true
  rm -f /etc/opt/chrome/policies/managed/pocketdesk.json \
        /etc/opt/chrome/policies/managed/pocketdesk-downloads.json 2>/dev/null || true
  if ! grep -q 'mcp_servers.pocketlinux' "$HOME/.codex/config.toml" 2>/dev/null; then
    cat >> "$HOME/.codex/config.toml" <<'CODEXMCP'

# PocketLinux's desktop tools: appshot, click, type_text, press_key, scroll.
[mcp_servers.pocketlinux]
command = "python3"
args = ["/usr/local/bin/pocketlinux-mcp"]
startup_timeout_sec = 30
CODEXMCP
  fi
  # Claude Code is registered through its own command, so the file format stays its business.
  if command -v claude >/dev/null 2>&1 \
     && ! grep -q '"pocketlinux"' "$HOME/.claude.json" 2>/dev/null; then
    timeout --foreground --kill-after=2s 15s claude mcp add --scope user pocketlinux -- python3 /usr/local/bin/pocketlinux-mcp \
      >/tmp/pocketlinux-mcp-register.log 2>&1 || true
  fi
  # Any project folder gets it too, for agents that read a project-scoped file. One written
  # under the old name names a program that is gone, so it is replaced rather than kept.
  if grep -q '"pocketdesk"' "$HOME/Projects/.mcp.json" 2>/dev/null; then
    rm -f "$HOME/Projects/.mcp.json"
  fi
...
if command -v gnome-keyring-daemon >/dev/null 2>&1; then
  keyfile="$HOME/.pocketlinux/keyring-key"
  if [ ! -s "$keyfile" ]; then
    mkdir -p "$HOME/.pocketlinux"
...
/usr/local/bin/pocketlinux-menu || true
...
if command -v pulseaudio >/dev/null 2>&1; then
  ...
  audio_call pulseaudio --daemonize=yes --exit-idle-time=-1 --disable-shm=yes >/tmp/pocketlinux-pulse.log 2>&1 || true
  ...
  mkdir -p "$HOME/.pocketlinux"
  chmod 700 "$HOME/.pocketlinux" 2>/dev/null || true
  rm -f "$HOME/.pocketlinux/audio.sock" "$HOME/.pocketlinux/audio.port"
  if audio_call pactl load-module module-simple-protocol-unix rate=44100 format=s16le channels=2 \
      ...
      socket="$HOME/.pocketlinux/audio.sock" >/dev/null 2>&1; then
    chmod 600 "$HOME/.pocketlinux/audio.sock" 2>/dev/null || true
    echo "sound: private socket" >> /tmp/pocketlinux-pulse.log
  else
    echo "sound: falling back to the local port" >> /tmp/pocketlinux-pulse.log
    echo 4712 > "$HOME/.pocketlinux/audio.port"
  ...
  rm -f "$HOME/.pocketlinux/mic.pipe"
  mkfifo -m 600 "$HOME/.pocketlinux/mic.pipe" 2>/dev/null || true
  if [ -p "$HOME/.pocketlinux/mic.pipe" ]; then
    ...
    if audio_call pactl load-module module-pipe-source source_name=phone_mic \
        ...
        file="$HOME/.pocketlinux/mic.pipe" format=s16le rate=16000 channels=1 \
        >> /tmp/pocketlinux-pulse.log 2>&1; then
      if audio_call pactl set-default-source phone_mic >> /tmp/pocketlinux-pulse.log 2>&1; then
        echo "microphone: pipe source ready" >> /tmp/pocketlinux-pulse.log
      else
        echo "microphone: source loaded, but default selection failed (exit $?)" >> /tmp/pocketlinux-pulse.log
      fi
    else
      echo "microphone: pipe source setup failed (exit $?)" >> /tmp/pocketlinux-pulse.log
    fi
  fi
fi
...
def notify(title, body, critical=False):
    command = ["notify-send", "-a", "PocketLinux", "-i", "pocketlinux-linux"]
    if critical:
        command += ["-u", "critical"]
    try:
...
def offer_download(path, children):
    ...
    if path.suffix.lower() in (".crdownload", ".part", ".tmp", ".download") or path.name.startswith("."):
        return
    marker = Path(str(path) + ".pocketlinux-seen")
    try:
    ...
    if path.suffix.lower() == ".deb":
        ...
        try:
            ...
            children.append(subprocess.Popen(["/usr/local/bin/pocketlinux-install", str(path)],
    ...
    try:
        children.append(subprocess.Popen(["/usr/local/bin/pocketlinux-save", str(path)],
                                         stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,
                                         stderr=subprocess.DEVNULL))
    except OSError:
...
def load_childwatch():
    path = os.environ.get("POCKETLINUX_CHILDWATCH_HELPER", "/usr/local/bin/pocketlinux-childwatch.py")
...
def main(home=None, download=None, panel_delay=12, display_pid=None, inherited_pids=()):
    ...
    download = Path(os.environ["POCKETLINUX_DOWNLOAD_DIR"]) if download is None else Path(download)
    state = home / ".pocketlinux"
    ...
    with childwatch.ChildWakeup() as child_events, (state / "desktop-watch.lock").open("a") as lock:
        ...
        with open("/tmp/pocketlinux-tint2.log", "a", buffering=1) as panel_log:
...
/usr/local/bin/pocketlinux-window-guard watch >/tmp/pocketlinux-window-guard-start.log 2>&1 &
DESKTOP_CHILDREN+=("$!")
command -v dunst >/dev/null 2>&1 && dunst >/tmp/pocketlinux-dunst.log 2>&1 &
DESKTOP_CHILDREN+=("$!")
pcmanfm --desktop --profile LXDE >/tmp/pocketlinux-pcmanfm.log 2>&1 &
DESKTOP_CHILDREN+=("$!")
