...
set_wallpaper() {
  ...
  if [ -n "$chosen" ] && [ -r "$chosen" ]; then
    if ! zenity --question --title="Wallpaper" --width=430 \
      ...
      apply_wallpaper /usr/share/backgrounds/pocketlinux.jpg
...
menu() {
  ...
  if ! have zenity; then
    printf 'usage: pocketlinux-settings [theme-dark|theme-light|panel|appearance|sound|storage|software|downloads|about]\n'
    return 0
  fi
