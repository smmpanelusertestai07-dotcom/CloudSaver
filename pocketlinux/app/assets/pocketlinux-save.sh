...
#
# Usage: pocketlinux-save <file> [computer|phone|ask]
#        pocketlinux-save --where          print the setting in force
#        pocketlinux-save --phone-dir      print the phone folder, or say why there is none
set -u
HOME_DIR=${POCKETLINUX_HOME_DIR:-${HOME:-/home/coder}}
# The same folder Chrome is pointed at, so both halves agree without a second setting.
PHONE_DIR=${POCKETLINUX_PHONE_DOWNLOAD_DIR:-$HOME_DIR/Phone/Download/PocketLinux}
...
current() {
  case "${POCKETLINUX_DOWNLOAD_TARGET:-}" in
    computer|phone|ask) printf '%s' "$POCKETLINUX_DOWNLOAD_TARGET" ;;
    *) printf 'computer' ;;
  esac
}
...
case "${1:-}" in
  ...
  ""|-h|--help)
    say "usage: pocketlinux-save <file> [computer|phone|ask]"
