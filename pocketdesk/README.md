...
| Viewer | In-app RFB 3.8 client bound to `127.0.0.1:5901`. The desktop is born in the phone's orientation and kept the size of the screen; at 100 % it sits inside a small gap with a rounded blue border on a deep backdrop (portrait and landscape), pinch or Screen → Zoom to look closer, Fit to come back, Full screen to hide the controls |
...
| Browser | One browser. GNOME Web (Epiphany) is the instant, lightweight default; installing **Brave** (Chromium, Chrome Web Store extensions, official arm64 apt repo, Rewards/Wallet/VPN/AI off by policy) makes it the computer's browser everywhere (desktop, panel, every link and sign-in). Downloads land in `~/Downloads` inside Linux |
...
| Reliability | The desktop runs **without PRoot's seccomp accelerator by default** — the accelerator breaks Chromium/Electron signal-handler resets (`socket()`/`readlink()` return ENOSYS and the app aborts), which was the "ChatGPT goes back by itself". A Settings toggle re-enables it. Chromium apps also use `--no-zygote` |
...
| Home screen | Three tabs on a bottom bar: Home (state with Tux, Needs attention, mobile data meter, Your phone is compatible, **Linux only, on purpose** with the checked facts, the questions, every detail opening in place, branded dialogs throughout), Apps (AI desktop apps, Computer basics with one browser + Developer tools, what the browser can install), Settings (grouped, incl. Faster desktop; a dot only for what Settings can fix) |
