...
| Where downloads go | Everything downloaded inside the computer lands in `~/Downloads`, which nothing else on the phone can read and which needs no permission. **Settings → Files → Where downloads go** chooses what happens next: keep it here (default), copy it to the phone's own `Download` folder as well (needs Phone files), or ask each time. Also in the desktop under **Tools → Where downloads go**; the setting is one file inside the computer, read fresh per download, so a change takes effect at once. The Downloads watcher offers to install an ARM64 Windows program, refuses an Intel-only one, names a `.deb`, and ignores half-finished downloads |
| Mobile app development | Apps tab row: Java 21, Gradle, `adb`, `fastboot`, `aapt2`, `scrcpy`. Ubuntu's own `aapt2` is installed and Gradle pointed at it (`android.aapt2FromMavenOverride`), which is what closes the one real ARM64 Android-build gap — Google publishes `aapt2` for Intel Linux only. `pocketdesk-mobile` starts an Expo or Android project, builds, and build-installs-opens on the connected phone |
| A phone to test on | Android's Wireless debugging listens on the phone's own network and the container shares it, so `127.0.0.1` reaches the phone PocketDesk is running on; another phone on the same Wi-Fi works the same way. `pocketdesk-adb` pairs and connects; the MCP server adds eleven `phone_*` tools (devices, install, launch, screenshot, ui, tap, swipe, text, key, logcat, shell) so an AI agent here can build, install, open and work a real phone unaided. No emulator: that needs hardware virtualisation, which no app on an unrooted phone can have |
