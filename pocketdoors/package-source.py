#!/usr/bin/env python3
"""Package reproducible PocketAgent Doors source, without signing material or build/runtime caches.

Usage: python3 package-source.py /absolute/path/PocketAgent-Doors-source.zip
...
def package(root, destination):
    ...
    for required in ['app/AndroidManifest.xml', 'build.sh', 'app/src/com/pocketagent/doors/HomeActivity.java',
    ...
    try:
        with zipfile.ZipFile(temporary, 'x', zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
            for path in selected:
                ...
                info = zipfile.ZipInfo('pocketdoors/' + relative.as_posix())
